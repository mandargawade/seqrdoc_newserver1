...
class PdfGenerateWOXSENJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
       ...
       // dd($printer_name);
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            $x = 13;
            $y = 85; 
            $invisible_namestr = strtoupper($studentData[1]); //STUDENT NAME
            ...
            $x = 13;
            $y = 88; 
            $invisible_namestr = $studentData[0]; //STUDENT NAME
            $pdfBig->SetFont($Arial, 0, 8, '', false);
            $pdfBig->SetXY($x, $y);
        ...
        $y = 177;
        $nameOrg = '';
        $Totalchar = (int)strlen($studentData[1]);
        ...
        $nameOrg = str_repeat($studentData[1].' '.$studentData[0], 50);
        ...
        if($Totalchar > 30){
            ...
            $pdfBig->SetXY($x, $y);
            $pdfBig->Cell(0, 0, $studentData[1], 0, false, 'C');

            $pdf->SetFont($timesNewRomanB, '', 25, '', false);
            $pdf->SetXY($x, $y);
            $pdf->Cell(0, 0, $studentData[1], 0, false, 'C');
        }else{
        ...
        $programName  = explode(' ',$studentData[2]);
        $part1 = '';
        $part2 = '';
        ...
        $pdfBig->SetFont($timesNewRoman, '', 17, '', false);
        $pdfBig->SetXY($x, $y);
        $pdfBig->Cell(0, 0, "Woxen University", 0, false, 'L');
        ...
        $pdfBig->SetFont($timesNewRomanB, '', 17, '', false);
        $pdfBig->SetXY($x, $y);
        $pdfBig->Cell(0, 0, $studentData[3].",", 0, false, 'C');

        $pdf->SetFont($timesNewRomanB, '', 17, '', false);
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, $studentData[3].",", 0, false, 'C');
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id,$student_name)
    {
        ...
        $result = StudentTable::create(['serial_no'=>$serial_no,'student_name' =>$student_name ,'certificate_filename'=>$certName,'template_id'=>$template_id,'key'=>$key,'path'=>$urlRelativeFilePath,'created_at'=>$datetime,'created_by'=>$ses_id,'updated_at'=>$datetime,'updated_by'=>$ses_id,'status'=>$sts,'site_id'=>$auth_site_id]);
        }
