...
class PdfGenerateBestiuJob2
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$pdfBig->SetXY(12, 88);
            $pdfBig->Cell(186, 0, 'Mr./Ms. '.$candidate_name, 0, false, 'C');			
			$pdfBig->SetFont($oef, '', 13, '', false); 
            $pdfBig->SetXY(12, 104.5);
            //$pdfBig->Cell(186, 0, 'Son/Daughter of Shri./Smt. '.$father_name, 0, false, 'C');
			$pdfBig->MultiCell(186, 0, 'Son/Daughter of Shri./Smt. <span style="font-family:'.$arialMTblack.';">'.$father_name.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
            ...
            $pdfBig->SetXY(12, 127);
            //$pdfBig->Cell(186, 0, $degree, 0, false, 'C');
			$pdfBig->MultiCell(186, 0, '<span style="font-size:13;">Degree of </span>'.$degree, 0, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($oef, '', 13, '', false); 
            $pdfBig->SetXY(10, 140); 
            $pdfBig->Cell(190, 0, 'of this University on '.$completion_date.'.', 0, false, 'C');           	
			$pdfBig->SetXY(10, 152); 
            $pdfBig->Cell(190, 0, 'His/Her Overall Grade Point Average is '.$average.' out of 10.00', 0, false, 'C');			
			$pdfBig->SetXY(10, 162); 
			...
			$pdfBig->MultiCell(190, 0, 'He/She has been placed in <span style="font-family:'.$arialMTblack.';">'.$division.'</span> Division.', 0, 'C', 0, 0, '', '', true, 0, true);	
			$pdfBig->SetXY(45.5, 186); 
			...
			$pdf->SetXY(12, 88);
            $pdf->Cell(186, 0, 'Mr./Ms. '.$candidate_name, 0, false, 'C');			
			$pdf->SetFont($oef, '', 13, '', false); 
            $pdf->SetXY(12, 104.5);
            //$pdf->Cell(186, 0, 'Son/Daughter of Shri./Smt. '.$father_name, 0, false, 'C');
			$pdfBig->MultiCell(186, 0, 'Son/Daughter of Shri./Smt. <span style="font-family:'.$arialMTblack.';">'.$father_name.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetXY(10, 115.5);
            $pdf->Cell(190, 0, 'has successfully completed the requirements of course work for the', 0, false, 'C');
			$pdf->SetFont($oef, '', 18.5, '', false); 
            $pdf->SetXY(12, 127);
            //$pdf->Cell(186, 0, $degree, 0, false, 'C');
			$pdf->MultiCell(186, 0, '<span style="font-size:13;">Degree of </span>'.$degree, 0, 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($oef, '', 13, '', false); 
            $pdf->SetXY(10, 140); 
            $pdf->Cell(190, 0, 'of this University on '.$completion_date.'.', 0, false, 'C');            	
			$pdf->SetXY(10, 152); 
            $pdf->Cell(190, 0, 'His/Her Overall Grade Point Average is '.$average.' out of 10.00', 0, false, 'C');			
			$pdf->SetXY(10, 162); 
            $pdf->Cell(190, 0, 'and percentage of marks is '.$percentage.'.', 0, false, 'C');			
			$pdf->SetXY(10, 173); 
            //$pdf->Cell(190, 0, 'He/She has been placed in '.$division.' Division.', 0, false, 'C');	
			$pdf->MultiCell(190, 0, 'He/She has been placed in <span style="font-family:'.$arialMTblack.';">'.$division.'</span> Division.', 0, 'C', 0, 0, '', '', true, 0, true);	
			$pdf->SetXY(45.5, 186); 
