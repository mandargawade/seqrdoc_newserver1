...
class PdfGenerateUNEBCOEJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
       ...
       // $pdfBig->SetOverprint(true, true, 0);
		...
		$preview_serial_no=1;
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('UNEBCertCOE');
        if($previewPdf!=1){
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            if($previewPdf==1){
                if($previewWithoutBg!=1){
                    ...
                    $pdfBig->Image($template_img_generate, 0, 0, '210', '286', "JPG", '', 'R', true);
                }
            ...
            // add spot colors
            $pdf->AddSpotColor('Special', 30, 100, 90, 10);        // For Invisible
            ...
            //set background image
            $template_img_generate = public_path().'\\'.$subdomain[0].'\backend\canvas\bg_images\\'.$low_res_bg_verification;
			...
			// if($previewPdf == 1){ 	
                $x= 72;//68 //71
                $y = 272.5;//269 //266//282.5
                $yVerification = 273;//269
                $font_size=12;
                
                if($previewPdf!=1){
					$str = str_pad($card_serial_no, 7, '0', STR_PAD_LEFT);
				}else{
					$str = str_pad('0', 7, '0', STR_PAD_LEFT);	
				}				
            ...
            $centreAddress = $studentData[2];
            $indexNo = $studentData[3];
            $examYear = $studentData[4];
            $gender = $studentData[5];
            $entryCode = $studentData[6];
            $age = $studentData[7];
            $candidateName = $studentData[8];
         ...
         //
            ...
            */// exit;
            $resultFinal = $studentData[10];
            $aggregate = $studentData[11];
            ...
            $profilePicName= str_replace('/', '_', $indexNo);
                ...
                if(file_exists($profile_path_org)) {        
                    ...
                    $profilex = 165+5.6;//167.2 5.5
                    $profiley = 62+0.5;//58//+3.9

                    $profilexVerification = 167.2;
                    $profileyVerification = 60;
                    $profileWidth = 31;
                    $profileWidthVerification = 31;
                    $profileHeight = 44;
                    $pdfBig->image($profile_path_org,$profilex,$profiley,$profileWidth,$profileHeight,"",'','L',true,3600);

                    $pdf->image($profile_path_org,$profilexVerification,$profileyVerification,$profileWidthVerification,$profileHeight,"",'','L',true,3600);
                }
            ...
            $pdfBig->SetFont($universe, '', 10.6, '', false);
            $pdfBig->SetTextColor(0, 0, 0, 100);
            $text = 'This is to certify that the candidate named below, and whose';
            $pdfBig->setXY(21, 63);//17.2 70 63.5
            $pdfBig->MultiCell(168, 0, $text, 0, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($universe, '', 10.6, '', false);
            $pdfBig->SetTextColor(0, 0, 0, 100);
            $text = 'photograph appears, sat for the Uganda Certificate';
            $pdfBig->setXY(20, 67.5+1.5);
            $pdfBig->MultiCell(168, 0, $text, 0, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($universe, '', 10.6, '', false);
            $pdfBig->SetTextColor(0, 0, 0, 100);
            $text = 'of Education Examination in the year 2019, and qualified';
            $pdfBig->setXY(20, 73+1.5);
            $pdfBig->MultiCell(168, 0, $text, 0, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($universe, '', 10.6, '', false);
            $pdfBig->SetTextColor(0, 0, 0, 100);
            $text = 'for the award of the';
            $pdfBig->setXY(20.5, 78.5+1.5);
            $pdfBig->MultiCell(168, 0, $text, 0, 'C', 0, 0, '', '', true, 0, true);


            $pdfBig->SetFont($LUCIDACONSOLE, '', 11, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(82+5, 99.5+2);//99.5+5.5
            $pdfBig->MultiCell(169, 0, "DIVISION 4", '', 'L', 0, 1);


            $pdfBig->SetFont($LUCIDACONSOLE, '', 11, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(21, 109.8+4);//10
            $pdfBig->MultiCell(145, 0, "THE CANDIDATE REACHED THE GRADES SHOWN IN THE SUBJECTS NAMED", '', 'L', 0, 1);
           

            $pdfBig->SetFont($ARIALlMONOSPACEDMTBOLD, 'B', 11, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(17+3, 125.5+10-6);
            $pdfBig->MultiCell(100, 0, $candidateName, '', 'L', 0, 1);
            ...
            $pdfBig->SetXY(160, 219);
            $pdfBig->Rotate(90);
            
            //$pdfBig->MultiCell(100, 0, $candidateName, '', 'L', 0, 1);
            $pdfBig->Cell(5, 24.5, $candidateName);
            $pdfBig->Rotate(0);
            ...
            $pdfBig->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(119+3, 125.5+10-6);
            $pdfBig->MultiCell(145, 0, "DATE OF BIRTH: ".$date, '', 'L', 0, 1);
            ...
            $pdfBig->SetFont($ARIALlMONOSPACEDMTBOLD, 'B', 11, '', false);
            $pdfBig->SetTextSpotColor('Special', 100);
            //$pdfBig->setXY(119+3+35.2, 125.5+10+4-6);
            $pdfBig->SetXY(176, 219);
            $pdfBig->Rotate(90);
            $pdfBig->MultiCell(145, 0, $date, '', 'L', 0, 1);
            $pdfBig->Rotate(0);
            $pdfBig->SetOverprint(false, false, 0);
            $pdfBig->StopTransform();
            $pdfBig->SetTextColor(0,0,0);
            ...
            $pdfBig->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(17+3, 135+10-6);
            $pdfBig->MultiCell(100, 0, $centreName, '', 'L', 0, 1);

            $pdfBig->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(119+3, 135+10-6);
            $pdfBig->MultiCell(145, 0, "INDEX NUMBER: ".$indexNo, '', 'L', 0, 1);
            ...
            $pdfBig->SetFont($ARIALlMONOSPACEDMTBOLD, 'B', 11, '', false);
            $pdfBig->SetTextSpotColor('Special', 100);
            $pdfBig->SetXY(182, 219);
            $pdfBig->Rotate(90);
            //$pdfBig->setXY(119+3+58, 135+10-6);
            $pdfBig->MultiCell(145, 0, $indexNo, '', 'L', 0, 1);
            $pdfBig->Rotate(0);
            $pdfBig->SetOverprint(false, false, 0);
            $pdfBig->StopTransform();
            $pdfBig->SetTextColor(0,0,0);
            ...
            $pdfBig->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(17+3, 139+10.8-6);
            $pdfBig->MultiCell(150, 0, $centreAddress, '', 'L', 0, 1);

            $pdfBig->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(119+3, 139+10.8-6);
            $pdfBig->MultiCell(145, 0, "ENTRY CODE: ".$entryCode, '', 'L', 0, 1);
            ...
            for($i=1;$i<=10;$i++)
            {
                ...
                {
                    $pdfBig->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
                    $pdfBig->setXY(25.5+3.5, $yaxis);
                    $pdfBig->MultiCell(169, 0, strtoupper($subject), '', 'L', 0, 1);

                    /********INVISIBLE Start****/
                    $pdfBig->SetOverprint(true, true, 0);
                    $pdfBig->SetFont($ARIALlMONOSPACEDMTBOLD, 'B', 11, '', false);
                    $pdfBig->SetTextSpotColor('Special', 100);
                    $pdfBig->setXY(162, $yaxis);
                    ...
                    $pdfBig->setXY(125+3.5, $yaxis);
                    $pdfBig->MultiCell(169, 0, $result.' ('.strtoupper($numberInWord[$result]).')', '', 'L', 0, 1);
                    $yaxis = $yaxis + 5;
                    $subjectCount++;
            ...
            $pdfBig->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(21, $yaxis+3.5);
            $pdfBig->MultiCell(100, 0, 'SUBJECTS NAMED: '.strtoupper($numberInWord[$subjectCount]), '', 'L', 0, 1);

            $pdfBig->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(100+4, $yaxis+3.5);
            $pdfBig->MultiCell(145, 0, "SUBJECTS PASSED: ".strtoupper($numberInWord[$subjectsPassed]), '', 'L', 0, 1);
    
            $qrCodex = 168.5;
            $qrCodey = 250;
            $qrCodeyVerification = 250;
            $qrCodeWidth =18;
            $qrCodeHeight = 18;
            $pdfBig->write2DBarcode($codeContents, 'QRCODE,Q', $qrCodex+2, $qrCodey, $qrCodeWidth, $qrCodeHeight, $style1D, 'N');
            ...
            //$sign_secretary = public_path().'\\'.$subdomain[0].'\backend\canvas\images\\sign_secretary.png';   
            //$pdfBig->Image($sign_secretary, 8, 220, '72', '20', "PNG", '', 'R', true);

            //$pdfBig->ImageSVG( $sign_secretary,  8, 220, '72', '20',  $link = '',  $align = '',  $palign = '',  $border=1,  $fitonpage = false );

            //$sign_chairman = public_path().'\\'.$subdomain[0].'\backend\canvas\images\\sign_chairman.png';   
            //$pdfBig->Image($sign_chairman, 140, 222, '30', '10', "PNG", '', 'R', true);





            $pdf->SetFont($universe, '', 10.6, '', false);
            $pdf->SetTextColor(0, 0, 0, 100);
            $text = 'This is to certify that the candidate named below, and whose';
            $pdf->setXY(17.2, 60);
            $pdf->MultiCell(168, 0, $text, 0, 'C', 0, 0, '', '', true, 0, true);
            ...
            $pdf->SetFont($LUCIDACONSOLE, '', 11, '', false);
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(82, 97.8);
            $pdf->MultiCell(169, 0, "DIVISION 4", '', 'L', 0, 1);


            $pdf->SetFont($LUCIDACONSOLE, '', 11, '', false);
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(17, 109.8);
            $pdf->MultiCell(145, 0, "THE CANDIDATE REACHED THE GRADES SHOWN IN THE SUBJECTS NAMED", '', 'L', 0, 1);
           

            $pdf->SetFont($ARIALlMONOSPACEDMTBOLD, 'B', 13.5, '', false);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->setXY(17, 125.5);
            $pdf->MultiCell(100, 0, $candidateName, '', 'L', 0, 1);

            /********INVISIBLE Start****/
            $pdf->StartTransform();
            $pdf->SetOverprint(true, true, 0);
            $pdf->SetFont($ARIALlMONOSPACEDMTBOLD, 'B', 11, '', false);
            ...
            $pdf->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(119, 125.5);
            $pdf->MultiCell(145, 0, "DATE OF BIRTH: ".$date, '', 'L', 0, 1);
            ...
            $pdf->SetFont($ARIALlMONOSPACEDMTBOLD, 'B', 11, '', false);
            $pdf->SetTextSpotColor('Special', 100);
            //$pdf->setXY(119+35.2, 125.5+4);
            $pdf->SetXY(186, 212);
            $pdf->Rotate(90);
            $pdf->MultiCell(145, 0, $date, '', 'L', 0, 1);
            $pdf->Rotate(0);
            $pdf->SetOverprint(false, false, 0);
            $pdf->StopTransform();
            $pdf->SetTextColor(0,0,0);
            ...
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(17, 135);
            $pdf->MultiCell(100, 0, $centreName, '', 'L', 0, 1);

            $pdf->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(119, 135);
            $pdf->MultiCell(145, 0, "INDEX NUMBER: ".$indexNo, '', 'L', 0, 1);
            ...
            $pdf->SetXY(190, 212);
            $pdf->Rotate(90);
            $pdf->MultiCell(145, 0, $indexNo, '', 'L', 0, 1);
            $pdf->Rotate(0);
            $pdf->SetOverprint(false, false, 0);
            $pdf->StopTransform();
            $pdf->SetTextColor(0,0,0);
            ...
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(17, 139);
            $pdf->MultiCell(150, 0, $centreAddress, '', 'L', 0, 1);

            $pdf->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(119, 139);
            $pdf->MultiCell(145, 0, "ENTRY CODE: ".$entryCode, '', 'L', 0, 1);
            ...
            for($i=1;$i<=10;$i++)
            {
                ...
                {
                    $pdf->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
                    $pdf->setXY(25.5, $yaxis);
                    $pdf->MultiCell(169, 0, strtoupper($subject), '', 'L', 0, 1);

                    /********INVISIBLE Start****/
                    $pdf->SetOverprint(true, true, 0);
                    $pdf->SetFont($ARIALlMONOSPACEDMTBOLD, 'B', 11, '', false);
                    $pdf->SetTextSpotColor('Special', 100);
                    $pdf->setXY(159.5, $yaxis);
                    ...
                    $pdf->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
                    $pdf->setXY(125, $yaxis);
                    $pdf->MultiCell(169, 0, $result.' ('.strtoupper($numberInWord[$result]).')', '', 'L', 0, 1);
                    $yaxis = $yaxis + 5;
                    $subjectCount++;
            ...
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(17, 207);
            $pdf->MultiCell(100, 0, 'SUBJECTS NAMED: '.strtoupper($numberInWord[$subjectCount]), '', 'L', 0, 1);

            $pdf->SetFont($LUCIDACONSOLE, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(100, 207);
            $pdf->MultiCell(145, 0, "SUBJECTS PASSED: ".strtoupper($numberInWord[$subjectsPassed]), '', 'L', 0, 1);
            ...
            $qrCodeWidth =18;
            $qrCodeHeight = 18;
            $pdf->write2DBarcode($codeContents, 'QRCODE,Q', $qrCodex, $qrCodeyVerification, $qrCodeWidth, $qrCodeHeight, $style1D, 'N');


            $pdf->SetFont('Arial', '', 1.5, '', false);
            $pdf->setXY(166, 268);//258
            //$pdf->MultiCell(19, 0, trim($candidateName).' GA '.$aggregate, '', 'C', 0, 1);
            $pdf->MultiCell(19, 0, trim($candidateName), '', 'C', 0, 1);

            $pdf->Image("$tmpDir/" . $name."".$ghost_font_size.".png", $ghostImagex, $ghostImageyVerification, $w, $ghostImageHeight, "PNG", '', 'C', true, 3600);

            $sign_secretary = public_path().'\\'.$subdomain[0].'\backend\canvas\images\\sign_secretary.png';   
            $pdf->Image($sign_secretary, 8, 220, '72', '20', "PNG", '', 'R', true);

            $sign_chairman = public_path().'\\'.$subdomain[0].'\backend\canvas\images\\sign_chairman.png';   
            $pdf->Image($sign_chairman, 140, 222, '30', '10', "PNG", '', 'R', true);
            ...
            if($previewPdf!=1){
                ...
                $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'UNEBCertCOE',$admin_id,$card_serial_no);
       ...
       if($previewPdf!=1){
        $this->updateCardNo('UNEBCertCOE',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'UNEBCertCOE'.date("Ymdhms")).'.pdf';
