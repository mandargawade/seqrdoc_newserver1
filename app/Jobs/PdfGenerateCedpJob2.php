...
class PdfGenerateCedpJob2
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$high_res_bg="CEDP_Certificate.jpg"; // CEDP_Certificate, CEDP_Certificate_BG
			$low_res_bg="CEDP_Certificate.jpg";
			$pdfBig->AddPage();
			...
			if($previewPdf==1){
				if($previewWithoutBg!=1){
					...
					//$pdfBig->Image($template_img_generate, 0, 0, '210', '297', "JPG", '', true, false);
				}
				$date_font_size = '11';
				$date_nox = 13;
				$date_noy = 40;
			...
			$uv_logo = public_path()."/".$subdomain[0]."/backend/templates/186/cedp uv.png";
			$logox = 128;
			$logoy = 43;
			$logoWidth = 48.683333333;
			$logoHeight = 13.229166667;
			...
			$date_of_issuance = trim($studentData[1]);
			$csn = trim($studentData[2]);
			$can_no = trim($studentData[3]);
			$candidate_name = trim($studentData[4]);        
			$father_name = trim($studentData[5]);
			$mother_name = trim($studentData[6]);
			$role = trim($studentData[7]);
			$uid_no = trim($studentData[8]);
			$session = trim($studentData[9]);
			$Level = trim($studentData[10]);
			/*
			...
			*/			
            ...
            $pdfBig->SetXY(22, 47.5);
            $pdfBig->Cell(0, 0, 'CSN:', 0, false, 'L');
            $pdfBig->SetXY(22, 54.5);
            $pdfBig->Cell(0, 0, "CAN:", 0, false, 'L');            
			$pdfBig->SetXY(33, 47.5);
			$pdfBig->MultiCell(30, 3, $csn,'B', 'L', 0, 0);
			$pdfBig->SetXY(33, 54.5);
			$pdfBig->MultiCell(30, 3, $can_no,'B', 'L', 0, 0);
			//$pdfBig->writeHTML('<u>'.$can_no.'</u>', true, 0, true, 0);
            
			$pdfBig->SetFont($mtcorsva, '', 18.5, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(79, 95);
			$pdfBig->MultiCell(0, 0, 'This is to certify that', 0, 'L', 0, 0);
			$pdfBig->SetXY(25.5, 108);
			$pdfBig->MultiCell(0, 0, 'Mr/Ms.', 0, 'L', 0, 0);
			$pdfBig->SetXY(25.5, 118);
			$pdfBig->MultiCell(0, 0, 'son/daughter of Mr.', 0, 'L', 0, 0);
			$pdfBig->SetXY(25.5, 129);
			$pdfBig->MultiCell(0, 0, '& Mrs.', 0, 'L', 0, 0);
			$pdfBig->SetXY(25.5, 162);
			$pdfBig->MultiCell(0, 0, 'UID No.', 0, 'L', 0, 0);
			$pdfBig->SetXY(104, 162);
			$pdfBig->MultiCell(0, 0, 'for session', 0, 'L', 0, 0);
			$pdfBig->SetFont($mtcorsva, '', 19.5, '', false);
			$pdfBig->SetXY(24, 138);
			$pdfBig->MultiCell(160, 0, 'has successfully completed the assessment of the role of',0, 'C', 0, 0);
			$pdfBig->SetFont($mtcorsva, '', 19, '', false);
			$pdfBig->SetXY(25.5, 171.5);
			$pdfBig->MultiCell(155, 0, 'conforming to National Skill Qualifications Framework Level-',0, 'L', 0, 0);
			$pdfBig->SetFont($mtcorsva, '', 18, '', false);
			$pdfBig->SetXY(24.5, 181.5);
			$pdfBig->MultiCell(160, 0, 'Skilled and trained at Council of Education and Development',0, 'C', 0, 0);
			$pdfBig->SetFont($mtcorsva, '', 17, '', false);
			$pdfBig->SetXY(41, 191);
			$pdfBig->MultiCell(160, 0, 'Programmes (CEDP Skill Institute), TPID No. TP004002',0, 'L', 0, 0);
			
			$pdfBig->SetXY(47, 114);
			$pdfBig->MultiCell(135, 1, '', 'T', 'L', 0, 0); //Mr. & Mrs.
			$pdfBig->SetXY(78, 124);
			$pdfBig->MultiCell(104, 1, '', 'T', 'L', 0, 0); //son, daughter of Mr.
			$pdfBig->SetXY(44.6, 134.3);
			$pdfBig->MultiCell(137, 1, '', 'T', 'L', 0, 0); //& Mrs.
			$pdfBig->SetXY(27, 155);
			$pdfBig->MultiCell(155, 1, '', 'T', 'L', 0, 0); //role
			$pdfBig->SetXY(49, 168);
			$pdfBig->MultiCell(53, 1, '', 'T', 'L', 0, 0); //uid no
			$pdfBig->SetXY(132, 167.5);
			$pdfBig->MultiCell(50, 1, '', 'T', 'L', 0, 0); //session
			$pdfBig->SetXY(177.2, 178);
			$pdfBig->MultiCell(5.4, 1, '', 'T', 'L', 0, 0); //level
			
			$pdfBig->SetFont($arial, '', 16, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->SetXY(54, 107);
            $pdfBig->MultiCell(166, 0, $candidate_name, 0, 'L', 0, 0);
             
			$pdfBig->SetFont($arial, '', 16, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(84, 117);	
			$pdfBig->MultiCell(60, 0, $father_name,0, 'L', 0, 0);
			
			$pdfBig->SetFont($arial, '', 16, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(54, 128);	
			$pdfBig->MultiCell(70, 0, $mother_name,0, 'L', 0, 0);
			
			$pdfBig->SetFont($arial, '', 16, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(24, 147);	
			$pdfBig->MultiCell(160, 0, $role,0, 'C', 0, 0);
			
			$pdfBig->SetFont($arial, '', 16, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(49, 161);	
			$pdfBig->MultiCell(53, 0, $uid_no, 0, 'C', 0, 0);
			
			$pdfBig->SetFont($arial, '', 16, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(133, 161);	
			$pdfBig->MultiCell(50, 0, $session, 0, 'C', 0, 0);
			
			$pdfBig->SetFont($arial, '', 16, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(177, 171);	
			$pdfBig->MultiCell(0, 0, $Level, 0, 'L', 0, 0);
			
			$pdfBig->SetFont($arial, '', 13, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(33, 245);	
			$pdfBig->MultiCell(30, 0, $date_of_issuance,0, 'C', 0, 0);
       ...
       $msg = '';
        ...
        $file_name =  str_replace("/", "_",'cedp_certificate'.date("Ymdhms")).'.pdf';
       ...
       $filename = public_path().'/backend/tcpdf/examples/'.$file_name;
        ...
        if($previewPdf!=1){
            ...
            $template_name="cedp_certificate";
