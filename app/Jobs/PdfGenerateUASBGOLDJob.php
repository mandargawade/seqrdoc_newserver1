     ...
     $admin_id = \Auth::guard('admin')->user()->toArray();
       ...
       $timesNewRoman = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Times-New-Roman.TTF', 'TrueTypeUnicode', '', 96);
        ...
        {   
             ...
             $serial_no = trim($fetch_degree_data[$excel_row][0]);
            $path=public_path().'/'.$subdomain[0].'/backend/DC_Photo/';
             $file =trim($fetch_degree_data[$excel_row][1]); 
      ...
      $serial_no_split = (string)($serial_no);
       ...
       foreach ($strArr as $character) {
        ...
        $minus=0;
        if(trim($fetch_degree_data[$excel_row][6])=="Bachelor of Technology (Agricultural Engineering)"){
            $minus=1.5;
        }
        $strFont = '18';
        $strX = 85.1;
        ...
        if(trim($fetch_degree_data[$excel_row][6])!="Bachelor of Science (Sericulture)"&&trim($fetch_degree_data[$excel_row][6])!="Bachelor of Science (Agriculture)"&&trim($fetch_degree_data[$excel_row][6])!="Bachelor of Science (Agricultural Biotechnology)"){
        $strFont = '23';
        $strX = 55;
        $strY = 75+1.2-$minus;
        $str=trim($fetch_degree_data[$excel_row][7]);
        ...
        $strX = 55;
        $strY = 75+1.2-$minus;
        $str1=trim($fetch_degree_data[$excel_row][7])." ";
        ...
        $strX = 55;
        $strY = 196.7;
        $str = $fetch_degree_data[$excel_row][6];
        ...
        if(trim($fetch_degree_data[$excel_row][6])=="Master of Technology (Agricultural Engineering)"&&trim($fetch_degree_data[$excel_row][6])!="IN SOIL AND WATER ENGINEERING"){
        ...
        $strFontB = '20';
        $strFont = '20';
        $str1=$fetch_degree_data[$excel_row][7]." ";
        $str2="CAVÃPÀj¸À¯ÁVzÉ";
        ...
        //Template 02
        if(trim($fetch_degree_data[$excel_row][12])=="UG"){    
            $strY = 44;
            if(trim($fetch_degree_data[$excel_row][13])!="1"){    
            $strFont = '30';
            $strX = 85.1;
            ...
            if(strlen(trim($fetch_degree_data[$excel_row][11]))>90){
                $strFont = '16.5';
                $strY = $strY+14.3;//69.3
            }else{
               $strFont = '20'; 
            ...
            $strX = 13;
            $strY = $strY+10;
            $str = trim($fetch_degree_data[$excel_row][5]);
            $pdf->SetFont($nudi01eb, 'B', $strFont, '', false);
            $pdf->SetTextColor(0,0,0,100,false,'');
            $pdf->SetXY(16.8, $strY);
            $pdf->Cell(0, 0, $str, 0, false, 'L');
            ...
            $str = trim($fetch_degree_data[$excel_row][4]);
            $pdf->SetFont($nudi01eb, 'B', $strFont, '', false);
            $pdf->SetTextColor(0,0,0,100,false,'');
            $pdf->SetXY(125, $strY);
            $pdf->Cell(0, 0, $str, 0, false, 'L');
        ...
        $str = trim($fetch_degree_data[$excel_row][9])." ".trim($fetch_degree_data[$excel_row][7]);
        ...
        if(strlen($str)>120){
            ...
            $strY=80;    
            $str1=$strArr[1];
            $str2=" ".trim($fetch_degree_data[$excel_row][5]);
            $str3="£ÉÃ ¸Á°£À°è 10.00 CAPÀUÀ½UÉ ¸ÀgÁ¸Àj";
            $str4 = trim($fetch_degree_data[$excel_row][4]);
            $result = $this->GetStringPositions(
                    array(
       ...
       $strFont = '17.9';
        ...
        if($fetch_degree_data[$excel_row][12]!="UG"){
        $strFont = '17.9';
        $str=trim($fetch_degree_data[$excel_row][6])." ".trim($fetch_degree_data[$excel_row][8]);
        ...
        if(strlen($str)>70){
            ...
            $str3=trim($fetch_degree_data[$excel_row][5]);
         $result = $this->GetStringPositions(
                    array(
        ...
        $strX = 55;
        $strY = 219;
        $str = trim($fetch_degree_data[$excel_row][6]);
        $pdf->SetFont($timesNewRomanBI, '', $strFont, '', false);
        $pdf->SetTextColor(0,0,0,100,false,'');
        $pdf->SetXY(10, $strY-$z);
        $pdf->Cell(0, 0, $str, 0, false, 'C');
       ...
       $str = $fetch_degree_data[$excel_row][2];
        ...
        $pdfBig->StopTransform();        
         ...
         if($previewPdf!=1){     
            ...
            $student_table_id = $this->addCertificate($serial_no, $certName, $dt,$template_data['id'],$admin_id);
            ...
            $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,$template_data['template_name'],$admin_id,$student_table_id);
