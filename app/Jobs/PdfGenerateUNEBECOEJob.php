...
class PdfGenerateUNEBECOEJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $ghostImgArr = array();
        $pdfBig = new TCPDF('P', 'mm', array('206', '314.8'), true, 'UTF-8', false);
        ...
        // add spot colors
        $pdfBig->AddSpotColor('Special', 30, 100, 90, 10);        // For Invisible //Spot Red
        ...
        $CourierNew = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\CourierNew.ttf', 'TrueTypeUnicode', '', 96);
        ...
        $high_res_bg="uneb_bg_ECOE.jpg"; 
        $low_res_bg="uneb_bg_ECOE.jpg";
        $template_img_generate = public_path().'\\'.$subdomain[0].'\backend\canvas\bg_images\\'.$high_res_bg;

        $low_res_bg_v="uneb_bg_ECOE_Verification.jpg";
        $template_img_generate_v = public_path().'\\'.$subdomain[0].'\backend\canvas\bg_images\\'.$low_res_bg_v;  

        //print_r($studentDataOrg);

        if($previewPdf!=1){
        $cardDetails=$this->getNextCardNo('UNEBCertECOE');
        ...
        for($dataIndex=0;$dataIndex<count($studentDataOrg);$dataIndex++){
            ...
            $studentData=$studentDataOrg[$dataIndex];
            ...
            if($previewPdf==1){
                if($previewWithoutBg!=1){
                    ...
                    $pdfBig->Image($template_img_generate, 0, 0, '206', '314.8', "JPG", '', 'R', true);
                }
            ...
            $pdf = new TCPDF('L', 'mm', array('102', '203.2'), true, 'UTF-8', false);
            ...
            if($previewPdf!=1){
                ...
                $pdf->Image($template_img_generate_v, 0, 0, '203.2', '102', "JPG", '', 'R', true);
            }
            $pdf->setPageMark();
            }
            // if($previewPdf!=1){ 
            ...
            $date=$dateFormat[0].'/'.date('m',strtotime($dateFormat[1])).'/'.$dateFormat[2];
            // exit;
            ...
            $ghostImagex = 82;
            $ghostImagey = 96.5;//85
            $ghostImageyVerification = 91.9;
            //$ghostImageWidth = 55;//68
            $ghostImageHeight = 6.3;//9.8
            ...
            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(34, 28);//21
            $pdfBig->MultiCell(100, 0, strtoupper($candidateName), '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(137.5, 28);
            $pdfBig->MultiCell(145, 0, strtoupper($indexNo), '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(34, 32);
            $pdfBig->MultiCell(100, 0, strtoupper($centreName), '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(137.5, 32);
            $pdfBig->MultiCell(145, 0, $centreAddress, '', 'L', 0, 1);


            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(34, 36);
            $pdfBig->MultiCell(100, 0, 'ENTRY CODE:'.$entryCode, '', 'L', 0, 1);
            ...
            {   
                ...
                $subjectCodeInt=$studentData[$count++];
                $subject = $subjectCode[$subjectCodeInt];
                $result = $studentData[$count++];

                $subjectInitial=substr($subjectCodeInt, 0, 1);
                
                if($subject != "")
                {
                    //echo $studentData[$count++];//$subjectInitial=substr($studentData[$count++], 0, 1);
                    //echo "<br>";
                    $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
                    $pdfBig->setXY(26.5, $yaxis);
                    $pdfBig->MultiCell(169, 0, $subjectInitial.'  '.strtoupper($subject), '', 'L', 0, 1);
                    
                    $pdfBig->setXY(140.7, $yaxis);
                    if($result=="X"||$result=="x"){
                    $pdfBig->MultiCell(169, 0, 'X  (ABSENT)', '', 'L', 0, 1);
            ...
            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(26, $yaxis+4.5);
            $pdfBig->MultiCell(100, 0, 'GRADE AGGREGATE   '.$aggregate, '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(117.5, $yaxis+4.5);
            $pdfBig->MultiCell(145, 0, '***  RESULT  '.$resultFinal.'  ***', '', 'L', 0, 1);
            ...
            $pdfBig->SetOverprint(true, true, 0);
            $pdfBig->SetFont($courierNewBold, 'B', 11.1);
            $pdfBig->SetTextSpotColor('Special', 100);
            $pdfBig->SetXY(179, 96);
            $pdfBig->Rotate(90);
            $pdfBig->Cell(16, 21, $candidateName);//13
            
            $pdfBig->SetXY(179, 96);
            $pdfBig->Cell(16, 29, trim($indexNo).'    GA '.$aggregate); //21
            $pdfBig->Rotate(0);
            ...
            $pdf->SetFont($CourierNew, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->setXY(33, 23);
            $pdf->MultiCell(100, 0, strtoupper($candidateName), '', 'L', 0, 1);

            $pdf->SetFont($CourierNew, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(136.5, 23);
            $pdf->MultiCell(145, 0, strtoupper($indexNo), '', 'L', 0, 1);

            $pdf->SetFont($CourierNew, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->setXY(33, 27);
            $pdf->MultiCell(100, 0, strtoupper($centreName), '', 'L', 0, 1);

            $pdf->SetFont($CourierNew, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(136.5, 27);
            $pdf->MultiCell(145, 0, $centreAddress, '', 'L', 0, 1);


            $pdf->SetFont($CourierNew, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->setXY(33, 31);
            $pdf->MultiCell(100, 0, 'ENTRY CODE:'.$entryCode, '', 'L', 0, 1);
            ...
            for($i=1;$i<=10;$i++)
            {
                ...
                if($subject != "")
                {
                    ...
                    $pdf->MultiCell(169, 0, $subjectInitial.'  '.strtoupper($subject), '', 'L', 0, 1);

                    $pdf->setXY(139.7, $yaxis);
                    if($result=="X"||$result=="x"){
                    $pdf->MultiCell(169, 0, 'X  (ABSENT)', '', 'L', 0, 1);
            ...
            $pdf->SetFont($CourierNew, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->setXY(25, $yaxis + 4.5);
            $pdf->MultiCell(100, 0, 'GRADE AGGREGATE   '.$aggregate, '', 'L', 0, 1);

            $pdf->SetFont($CourierNew, '', 11.1, '', false);
            $pdf->SetTextColor(0, 0, 0, $opacity);
            $pdf->setXY(116, $yaxis + 4.5);
            $pdf->MultiCell(145, 0, '***  RESULT  '.$resultFinal.'  ***', '', 'L', 0, 1);
            ...
            $pdf->StartTransform();
            $pdf->SetOverprint(true, true, 0);
            $pdf->SetFont($courierNewBold, 'B', 11.1);
            $pdf->SetTextSpotColor('Special', 100);
            $pdf->SetXY(172, 92);
            $pdf->Rotate(90);
            $pdf->Cell(5, 31.5, $candidateName);//24.5
            
            $pdf->SetXY(172, 92);
            $pdf->Cell(5, 38.5, trim($indexNo).'    GA '.$aggregate); //31.5
            $pdf->Rotate(0);
            $pdfBig->SetOverprint(false, false, 0);
            $pdf->StopTransform();
            ...
            $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'UNEBCertECOE',$admin_id,$card_serial_no);
            ...
            // if($previewPdf == 1){  
                ...
                ///echo $z;
                //echo $dataIndex;
                if(isset($studentDataOrg[$dataIndex])){
            ...
            $ghost_font_size = '11';//13
            $ghostImagex = 81;
            $ghostImagey = 89.5+$leaf2Offset;
            $ghostImageyVerification = 91.9;
            //$ghostImageWidth = 55;//68
            $ghostImageHeight = 6.5;//9.8
            $name = substr(str_replace(' ','',strtoupper($nameOrg)), 0, 6);
            $tmpDir = $this->createTemp(public_path().'\backend\images\ghosttemp\temp');
            $w = $this->CreateMessage($tmpDir, $name ,$ghost_font_size,'');
            ...
            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(34, 21+$leaf2Offset);
            $pdfBig->MultiCell(100, 0, strtoupper($candidateName), '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(137.5, 21+$leaf2Offset);
            $pdfBig->MultiCell(145, 0, strtoupper($indexNo), '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(34, 25+$leaf2Offset);
            $pdfBig->MultiCell(100, 0, strtoupper($centreName), '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(137.5, 25+$leaf2Offset);
            $pdfBig->MultiCell(145, 0, $centreAddress, '', 'L', 0, 1);


            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(34, 29+$leaf2Offset);
            $pdfBig->MultiCell(100, 0, 'ENTRY CODE:'.$entryCode, '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(71, 29+$leaf2Offset);
            $pdfBig->MultiCell(145, 0, 'DATE OF BIRTH:'.$date, '', 'L', 0, 1);

           
            $pdfBig->SetTextColor(0, 0, 0);
            $yaxis = 37+$leaf2Offset;
            $count = 12;//11
            $subjectCount = 0;
            $subjectsPassed=0; 
            for($i=1;$i<=10;$i++)
            {
                ...
                {
                    $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
                    $pdfBig->setXY(25.5, $yaxis);
                    $pdfBig->MultiCell(169, 0, $subjectInitial.'  '.strtoupper($subject), '', 'L', 0, 1);

                    $pdfBig->setXY(139.7, $yaxis);
                    if($result=="X"||$result=="x"){
                    $pdfBig->MultiCell(169, 0, 'X  (ABSENT)', '', 'L', 0, 1);
            ...
            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(25, $yaxis+4.5);
            $pdfBig->MultiCell(100, 0, 'GRADE AGGREGATE   '.$aggregate, '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(116, $yaxis+4.5);
            $pdfBig->MultiCell(145, 0, '***  RESULT  '.$resultFinal.'  ***', '', 'L', 0, 1);

    
            $qrCodex = 10;
            $qrCodey = 15+$leaf2Offset;
            $qrCodeWidth =18;
            $qrCodeHeight = 18;
            $pdfBig->write2DBarcode($codeContents, 'QRCODE,Q', $qrCodex, $qrCodey, $qrCodeWidth, $qrCodeHeight, $style1D, 'N');

            $pdfBig->SetFont('Arial', '', 1.5, '', false);
            $pdfBig->setXY(10, 33+$leaf2Offset);//258
            $pdfBig->MultiCell(19, 0, trim($candidateName).' GA '.$aggregate, '', 'C', 0, 1);
            ...
            $pdfBig->SetFont($courierNewBold, 'B', 11.1);
            $pdfBig->SetTextSpotColor('Special', 100);
            $pdfBig->SetXY(179, 89+$leaf2Offset);
            $pdfBig->Rotate(90);
            $pdfBig->Cell(16, 21, $candidateName);//13
            ...
            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(34, 21+$leaf3Offset);
            $pdfBig->MultiCell(100, 0, strtoupper($candidateName), '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(137.5, 21+$leaf3Offset);
            $pdfBig->MultiCell(145, 0, strtoupper($indexNo), '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(34, 25+$leaf3Offset);
            $pdfBig->MultiCell(100, 0, strtoupper($centreName), '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(137.5, 25+$leaf3Offset);
            $pdfBig->MultiCell(145, 0, $centreAddress, '', 'L', 0, 1);


            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(34, 29+$leaf3Offset);
            $pdfBig->MultiCell(100, 0, 'ENTRY CODE:'.$entryCode, '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(71, 29+$leaf3Offset);
            $pdfBig->MultiCell(145, 0, 'DATE OF BIRTH:'.$date, '', 'L', 0, 1);

           
            $pdfBig->SetTextColor(0, 0, 0);
            $yaxis = 37+$leaf3Offset;
            $count = 12;//11
            $subjectCount = 0;
            $subjectsPassed=0; 
            for($i=1;$i<=10;$i++)
            ...
            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->setXY(25, 80+$leaf3Offset);
            $pdfBig->MultiCell(100, 0, 'GRADE AGGREGATE   '.$aggregate, '', 'L', 0, 1);

            $pdfBig->SetFont($CourierNew, '', 11.1, '', false);
            $pdfBig->SetTextColor(0, 0, 0, $opacity);
            $pdfBig->setXY(116, 80+$leaf3Offset);
            $pdfBig->MultiCell(145, 0, '***  RESULT  '.$resultFinal.'  ***', '', 'L', 0, 1);

    
            $qrCodex = 10;
            $qrCodey = 15+$leaf3Offset;
            $qrCodeWidth =18;
            $qrCodeHeight = 18;
            $pdfBig->write2DBarcode($codeContents, 'QRCODE,Q', $qrCodex, $qrCodey, $qrCodeWidth, $qrCodeHeight, $style1D, 'N');

            $pdfBig->SetFont('Arial', '', 1.5, '', false);
            $pdfBig->setXY(10, 33+$leaf3Offset);//258
            $pdfBig->MultiCell(19, 0, trim($candidateName).' GA '.$aggregate, '', 'C', 0, 1);
            ...
            $pdfBig->SetFont($courierNewBold, 'B', 11.1);
            $pdfBig->SetTextSpotColor('Special', 100);
            $pdfBig->SetXY(179, 89+$leaf3Offset);
            $pdfBig->Rotate(90);
            $pdfBig->Cell(16, 21, $candidateName);//13
       ...
       if($previewPdf!=1){
        $this->updateCardNo('UNEBCertECOE',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'UNEBCertECOE'.date("Ymdhms")).'.pdf';
