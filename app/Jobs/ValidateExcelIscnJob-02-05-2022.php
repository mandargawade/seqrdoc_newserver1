...
class ValidateExcelIscnJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if($dropdown_template_id==1){
            if($columnsCount1==20){
                //$photo_col=23;
				$columnArr=array("Unique_Id", "Name_o_the_Student", "Mother's_Name", "Exam_Category", "Gender", "Medium", "IRN_Number", "Roll_Number", "Enrollment_Number", "HPE_Grades", "Incentives", "AGPV", "Semester_Credits_(Ci)", "SGPA-T(Out of 10)", "SGPA$(Out of 10)", "Group_Total(Out of 450)", "Marks_Obtained(Out of 600)", "Result", "Number_of_Attempts", "Date");
		...
		elseif($dropdown_template_id==2){
            if($columnsCount1==17){
                $columnArr=array("Unique_Id", "Name_of_the_Student", "Mother's_Name", "Exam_Category", "Gender", "Medium", "IRN_Number", "Roll_Number", "Enrollment_Number", "Incentives", "AGPV", "Semester_Credits(CR)", "Marks_Obtained(Out of 625)", "SGPA(Out of 10)", "Result", "Number of Attempts", "Date");
                $mismatchColArr=array_diff($rowData1[0], $columnArr);
                if(count($mismatchColArr)>0){                    
                    return response()->json(['success'=>false,'type' => 'error', 'message' => 'Sheet1/Json : Column names not matching as per requirement. Please check columns : '.implode(',', $mismatchColArr)]);
                }
		...
		elseif($dropdown_template_id==3){
            if($columnsCount1==18){
                $columnArr=array("Unique_Id", "Specialization", "Name_of_the_Student", "Mother's_Name", "Exam_Category", "Gender", "Medium", "IRN_Number", "Roll_Number", "Enrollment_Number", "Incentives", "AGPV", "Semester_Credits", "SGPA(Out of 10)", "Marks_Obtained(Out of 625)", "Result", "Number_of_Attempts", "Date");
                $mismatchColArr=array_diff($rowData1[0], $columnArr);
                if(count($mismatchColArr)>0){                    
                    return response()->json(['success'=>false,'type' => 'error', 'message' => 'Sheet1/Json : Column names not matching as per requirement. Please check columns : '.implode(',', $mismatchColArr)]);
                }
