...
class PdfGenerateSecuraJob2
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $template_id=100; //$pdf_data['template_id'];
        ...
        $card_serial_no="";
        $log_serial_no = 1;
        //$cardDetails=$this->getNextCardNo('molwa');
        ...
        foreach ($studentDataOrg as $studentData) {            
            ...
            $ff_id=$studentData[0];
            $ff_name=$studentData[1];
            $fh_name=$studentData[2];
            $vw_name=$studentData[3];
            $post_office=$studentData[4];
            $ut_name=$studentData[5];
            $district=$studentData[6];
            $photo=$studentData[7];            
            $serial_no='ID_'.$studentData[0];
            $GUID=$studentData[0];
            ...
            if($studentData[$photo_col]!=''){
                //$ff_photo=public_path().'\\'.$subdomain[0].'\backend\templates\100\\'.$studentData[$photo_col];
                //$pdfBig->Image($ff_photo, 3, 18, 18.7, 19.5, 'png', '', true, false);    
                //$pdf->Image($ff_photo, 3, 18, 18.7, 19.5, 'png', '', true, false);
                /*$decoded_image=base64_decode($photo);
                $mimetype = $this->getImageMimeType($decoded_image); 
                $ff_photo = "<img src='data:image/$mimetype;charset=utf8;base64,$photo' width='18.7mm' height='19.5mm' />";                
                $pdfBig->SetXY(3, 18);
                $pdfBig->WriteHTML($ff_photo);
                $pdf->SetXY(3, 18);
                $pdf->WriteHTML($ff_photo);
                */    
                $pic = $this->getImage($photo);
                if ($pic !== false){
            ...
            if($previewPdf!=1){
                ...
                $pdf->output($myPath . DIRECTORY_SEPARATOR . 'ID_'.$certName, 'F');
        ...
        if($previewPdf!=1){
            //$this->updateCardNo('molwa',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'molwaIDcard'.date("Ymdhms")).'.pdf';
...
file_put_contents($local_server_file_path, file_get_contents($destinationURL));
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id)
    {
        ...
        $file1 = public_path().'/backend/temp_pdf_file/ID_'.$certName;
        $file2 = public_path().'/backend/pdf_file/ID_'.$certName;
        ...
        $pdfActualPath=public_path().'/'.$subdomain[0].'/backend/pdf_file/ID_'.$certName;
        ...
        $result = SbStudentTable::create(['serial_no'=>$serial_no,'certificate_filename'=>'ID_'.$certName,'template_name'=>$template_id,'key'=>$key,'path'=>$urlRelativeFilePath,'created_at'=>$datetime,'created_by'=>$ses_id,'updated_at'=>$datetime,'updated_by'=>$ses_id,'status'=>$sts,'site_id'=>$auth_site_id]);
        }else{
        $resultu = StudentTable::where('serial_no',$serial_no)->update(['status'=>'0']);
        // Insert the new record
        
        $result = StudentTable::create(['serial_no'=>$serial_no,'certificate_filename'=>'ID_'.$certName,'template_name'=>$template_id,'key'=>$key,'path'=>$urlRelativeFilePath,'created_at'=>$datetime,'created_by'=>$ses_id,'updated_at'=>$datetime,'updated_by'=>$ses_id,'status'=>$sts,'site_id'=>$auth_site_id,'template_type'=>2]);
        }
