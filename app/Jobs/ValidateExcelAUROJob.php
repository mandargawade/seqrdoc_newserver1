    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        //$photo_col=$columnsCount1-1;
                if($dropdown_template_id==1){
                    if($columnsCount1==8){
                        $columnArr=array('UNIQUE ID','SR. NO.','ENROLLMENT NUMBER','STUDENT NAME','DEGREE','DEGREE SPECIALISATION','SCHOOL NAME','DEGREE/DIPLOMA');
				...
				elseif($dropdown_template_id==3){
                    if($columnsCount1==9){
                        $columnArr=array('Unique_id','Enroll_no','Serial_No','STUDENT NAME','DEGREE','Thesis_Entitled','School_Name','Degree/Diploma','Date_Year');														
                            // 'UNIQUE ID','SR. NO.','ENROLLMENT NUMBER','STUDENT NAME','DEGREE','DEGREE SPECIALISATION','SCHOOL NAME','DEGREE/DIPLOMA');
                        $mismatchColArr=array_diff($rowData1[0], $columnArr);
                        if(count($mismatchColArr)>0){
                            return response()->json(['success'=>false,'type' => 'error', 'message' => 'Sheet1/Json : Column names not matching as per requirement. Please check columns : '.implode(',', $mismatchColArr)]);
                        }
