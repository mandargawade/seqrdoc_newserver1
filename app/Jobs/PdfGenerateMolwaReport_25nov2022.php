...
class PdfGenerateMolwaReport
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if($report_name != 3 && $report_name != 4){
			...
			->select('ff_id', 'ff_name', 'father_or_husband_name', 'mother_name', 'village_or_ward')
			->get();		
		...
		if($report_name != 3 && $report_name != 4){
			...
			<tr><td width="35%">District : <span style="font-family:'.$nikosh.';font-size:20;color:black;">'.$district.'</span></td><td width="35%">Upazila/Thana : <span style="font-family:'.$nikosh.';font-size:20;color:black;">'.$upazila.'</span></td><td width="20%">ID CARD Qty : '.$record_count.'</td><td width="10%">'.$str_isalive.'</td></tr>
			<tr><td colspan="4" height="10"></td></tr>
			</table>
			');
			...
			$pdfBig->AddPage();
			//$pdfBig->SetXY(6,45);	
			$xval=6; //Table x position		
			$pdfBig->SetFont($arial, '', 9.5);		
			$pdfBig->SetWidths(array(10,23,45,31,26,28,36));
			//Table's Titles			
			...
			//Table's Titles			
			$pdfBig->SetFont($arialb, '', 9.5);                
			$pdfBig->Row(array('SL', 'Upazila/Thana', 'Total Certificate'), array('C', 'L','R'), array($arialb,$arialb), array('9.5','9.5','9.5'), $xval, $subject_length, 1);  
			$cnt=0;
			$pdfBig->SetTextColor(0, 0, 0);
			foreach ($data_s as $value) {
