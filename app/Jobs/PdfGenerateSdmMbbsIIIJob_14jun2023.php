...
class PdfGenerateSdmMbbsIIIJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$footer_note="SDM College Medical Sciences and Hospital, Sattur, Dharwad – 580009, Karnataka, India";
			...
			$pdfBig->SetXY($left_str_x, $subj_y);
			$pdfBig->setCellPaddings( $left = '', $top = 1, $right = '', $bottom = '');
			$pdfBig->MultiCell(135, 5, 'PERCENTAGE (UE+IA): ', 'L', 'R', 0, 0);
			$pdfBig->MultiCell(45, 5, $Percentage, 'LR', 'L', 0, 0);
			
			$pdf->SetXY($left_str_x, $subj_y);
			$pdf->setCellPaddings( $left = '', $top = 1, $right = '', $bottom = '');
			$pdf->MultiCell(135, 5, 'PERCENTAGE (UE+IA): ', 'L', 'R', 0, 0);
