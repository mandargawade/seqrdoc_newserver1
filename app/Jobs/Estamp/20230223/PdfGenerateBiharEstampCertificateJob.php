...
class PdfGenerateBiharEstampCertificateJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $rupeeForadian = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\RUPEE_FORADIAN.TTF', 'TrueTypeUnicode', '', 96);
        //$card_serial_no="";
        $preview_serial_no=1;
        $log_serial_no = 1;
        ...
        // foreach ($studentDataOrg as $studentData) {
            ...
            // }
                $fontSize = 12;
                $rupees = substr($studentDataOrg['Consideration Price (Rs.)'], 0, strpos($studentDataOrg['Consideration Price (Rs.)'], "("));
                ...
                $resultsQR = $studentDataOrg['Certificate No.'].' | '.$studentDataOrg['Certificate Issued Date'].' | '.$studentDataOrg['First Party'].' | '.$studentDataOrg['Second Party'].' | '.$studentDataOrg['Stamp Duty Paid (Rs.)'];
                ...
                // repeat Rotate microline of vertical date and time code
                $microlineEnrollment= $studentDataOrg['Certificate Issued Date'];
                $microlineEnrollment = preg_replace('/\s+/', '', $microlineEnrollment);
                $textArrayEnrollment = imagettfbbox(1.3, 0, public_path().'/'.$subdomain[0].'/backend/canvas/fonts/ArialB.TTF', $microlineEnrollment);
                $strWidthEnrollment = ($textArrayEnrollment[2] - $textArrayEnrollment[0]);
                $strHeightEnrollment = $textArrayEnrollment[6] - $textArrayEnrollment[1] / 1.3;
                $latestWidthEnrollment = 1380;
                ...
                $variableDateAndTime = substr($microlineEnrollmentstrRep,0,$microlineEnrollmentCharReq);
                //end of repeat Rotate microline code

                $variableDateAndTime = substr($variableDateAndTime, 0, 1207);


                // repeat Rotate microline for vertical party name code
                $microlineEnrollment= "FP ".$studentDataOrg['First Party']." SP ".$studentDataOrg['Second Party'];
                $microlineEnrollment = preg_replace('/\s+/', '', $microlineEnrollment);
                $textArrayEnrollment = imagettfbbox(1.3, 0, public_path().'/'.$subdomain[0].'/backend/canvas/fonts/ArialB.TTF', $microlineEnrollment);
                $strWidthEnrollment = ($textArrayEnrollment[2] - $textArrayEnrollment[0]);
                $strHeightEnrollment = $textArrayEnrollment[6] - $textArrayEnrollment[1] / 1.3;
                $latestWidthEnrollment = 1110;
                $microlineEnrollmentstrLength=strlen($microlineEnrollment);
                //width per character
                $microlineEnrollmentcharacterWd =$strWidthEnrollment/$microlineEnrollmentstrLength;
                //Required no of characters required in string to match width
                ...
                $microlineEnrollmentstrRep = str_repeat($microlineEnrollment, $repeatemicrolineEnrollmentCount);                
                //Cut string in required characters (final string)
                $variablePartyName = substr($microlineEnrollmentstrRep,0,$microlineEnrollmentCharReq);
                //end of repeat Rotate microline code

                $variablePartyName = strtoupper(substr($variablePartyName, 0, 887));




                // repeat microline for horizontal party name code
                $microlineContent= "FP ".$studentDataOrg['First Party']." SP ".$studentDataOrg['Second Party'];
                $microlineContent = preg_replace('/\s+/', '', $microlineContent);
                $textArrayContent = imagettfbbox(11, 0, public_path().'/'.$subdomain[0].'/backend/canvas/fonts/Arialb.TTF', $microlineContent);
                $strWidthContent = ($textArrayContent[2] - $textArrayContent[0]);
                $strHeightContent = $textArrayContent[6] - $textArrayContent[1] / 11;
                $latestWidthContent = 750;
                $microlineContentstrLength=strlen($microlineContent);
                //width per character
                $microlineContentcharacterWd =$strWidthContent/$microlineContentstrLength;
                //Required no of characters required in string to match width
                $microlineContentCharReq=$latestWidthContent/$microlineContentcharacterWd;
                $microlineContentCharReq=round($microlineContentCharReq);
                //No of time string should repeated
                $repeatemicrolineContentCount=$latestWidthContent/$strWidthContent;
                $repeatemicrolineContentCount=round($repeatemicrolineContentCount)+1;
                //Repeatation of string 
                $microlineEnrollmentstrRep = str_repeat($microlineContent, $repeatemicrolineContentCount);                
                //Cut string in required characters (final string)
                $arrayValue = substr($microlineEnrollmentstrRep,0,$microlineContentCharReq);
                //end of repeat microline code
                $arrayValue = strtoupper(substr($arrayValue, 0, 69));
                ...
                $pdfBig->setXY(16, 20);
                $pdfBig->MultiCell(15, 0, $html, '', 'L', 0, 1, '', '', true, 0, true);

                $pdfBig->SetFont($timesNewRoman, 'B', 12, '', false);
                $pdfBig->setXY(170, 20);
                $pdfBig->MultiCell(15, 0, $html, '', 'R', 0, 1, '', '', true, 0, true);
                $pdfBig->SetTextColor(0, 0, 0);


                $pdfBig->SetFont($timesNewRoman, 'B', 17, '', false);
                $pdfBig->setXY(15, 53);
                $pdfBig->MultiCell(180, 0, 'E-Stamp Details', '', 'C', 0, 1);
                ...
                $pdfBig->setXY(90, 66);
                $pdfBig->MultiCell(105, 0, $studentDataOrg['Certificate No.'], '', 'L', 0, 1);
                ...
                $pdfBig->setPageMark(); 
                $pdfBig->Image($qr_code_path, 20, 242, '23', '23', "png", '', true, false);
                $pdfBig->setPageMark();

                $pdfBig->SetTextColor(0, 0, 0, 10);
                $pdfBig->SetFont($timesNewRoman, '', 11, '', false);
                $pdfBig->setXY(17, 279);
                $pdfBig->MultiCell(178, 0, $arrayValue, '', '', 0, 1);

                $pdfBig->SetTextColor(0, 0, 0);
                $pdfBig->StartTransform();
                $pdfBig->SetFont($timesNewRoman, '', 1.3, '', false);
                $pdfBig->setXY(15, 283);
                $pdfBig->Rotate(90);
                $pdfBig->MultiCell(270, 0, $variableDateAndTime, '', '', 0, 1);
                $pdfBig->Rotate(0);
                $pdfBig->StopTransform();

                $pdfBig->StartTransform();
                $pdfBig->SetFont($timesNewRoman, '', 1.3, '', false);
                $pdfBig->setXY(195, 283);
                $pdfBig->Rotate(90);
                $pdfBig->MultiCell(270, 3, $variablePartyName, '', '', 0, 1);
                $pdfBig->Rotate(0);
                $pdfBig->StopTransform();
                ...
                $pdf->SetFont($timesNewRoman, 'B', 8, '', false);
                $html = '<span style="font-family:dejavusans;">&#8377;'.$rupees.'</span>';

                $pdf->setXY(16, 20);
                $pdf->MultiCell(15, 0, $html, '', 'L', 0, 1, '', '', true, 0, true);

                $pdf->SetFont($timesNewRoman, 'B', 12, '', false);
                $pdf->setXY(170, 20);
                $pdf->MultiCell(15, 0, $html, '', 'R', 0, 1, '', '', true, 0, true);
                ...
                $pdf->setXY(90, 66);
                $pdf->MultiCell(105, 0, $studentDataOrg ['Certificate No.'], '', 'L', 0, 1);
                ...
                $pdf->setPageMark(); 
                $pdf->Image($qr_code_path, 20, 242, '23', '23', "png", '', true, false);
                $pdf->setPageMark();

                $pdf->SetTextColor(0, 0, 0, 10);
                $pdf->SetFont($timesNewRoman, '', 11, '', false);
                $pdf->setXY(17, 279);
                $pdf->MultiCell(176, 5, $arrayValue, '', '', 0, 1);

                $pdf->SetTextColor(0, 0, 0);
                $pdf->StartTransform();
                $pdf->SetFont($timesNewRoman, '', 1.3, '', false);
                $pdf->setXY(15, 283);
                $pdf->Rotate(90);
                $pdf->MultiCell(270, 0, $variableDateAndTime, '', '', 0, 1);
                $pdf->Rotate(0);
                $pdf->StopTransform();
                ...
                $pdf->setXY(195, 283);
                $pdf->Rotate(90);
                $pdf->MultiCell(270, 3, $variablePartyName, '', '', 0, 1);
                $pdf->Rotate(0);
                $pdf->StopTransform();
