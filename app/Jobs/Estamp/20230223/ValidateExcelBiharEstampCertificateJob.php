...
class ValidateExcelBiharEstampCertificateJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $validateArr = ['Certificate No.', 'Certificate Issued Date', 'Consideration Price (Rs.)', 'First Party', 'Second Party'];

        foreach($validateArr as $arr){
            if(!in_array($arr, $data)){
                return response()->json(['success'=>false,'type' => 'error', 'message' => 'Sheet1/Json : Column names not matching as per requirement. Please check columns : Certificate No., Certificate Issued Date, Consideration Price (Rs.), First Party, Second Party']);
            }
        }
