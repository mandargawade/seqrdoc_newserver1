    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $startTimeLoader =  date('Y-m-d H:i:s');
            ...
            $pdfBig->SetTextColor(0, 0, 0, 100, false, '');
                ...
                if($fetch_degree_array[$excel_row][10] == 'NU'){
                   $invisible2str = $fetch_degree_array[$excel_row][15];
                }else{
                    if(is_float($fetch_degree_array[$excel_row][6])){
                        $cgpaFormat=number_format(trim($fetch_degree_array[$excel_row][6]),2);
                    }else{
                ...
                if($fetch_degree_array[$excel_row][10] == 'NU'){
                    ...
                    if(strpos($fetch_degree_array[$excel_row][15], 'With') !== false){

                         $gradestr = $fetch_degree_array[$excel_row][15].' ';
                    }else{
                ...
                if($fetch_degree_array[$excel_row][10] == 'NU'){
                   $microlineEnrollment = strtoupper(str_replace(' ','',$fetch_degree_array[$excel_row][1].strtoupper($fetch_degree_array[$excel_row][15]).$fetch_degree_array[$excel_row][8])); 
                }else{
                  if(is_float($fetch_degree_array[$excel_row][6])){
                        $cgpaFormat=number_format(trim($fetch_degree_array[$excel_row][6]),2);
                    }else{
                ...
                if($fetch_degree_array[$excel_row][10] == 'DB' || $fetch_degree_array[$excel_row][10] == 'INT'){
                    ...
                    if(strpos($fetch_degree_array[$excel_row][15], 'With') !== false){
                        ...
                        $grade_hindiy = 178.8;
                        $grade_hindix = 31.5; 
                    }else if($fetch_degree_array[$excel_row][15]=="First"){
                       $grade_hindiy = 178.8;
                        $grade_hindix = 49.5; 
                    }else{
                       $grade_hindiy = 178.8;
                ...
                if($fetch_degree_array[$excel_row][10] == 'NU'){
                  $grade_hindistr = trim($fetch_degree_array[$excel_row][16]);
                }else{
                ...
                $repeatx= 0;
                    ...
                    $name_repeaty = 242.9;
                     if($fetch_degree_array[$excel_row][10] == 'NU'){
                        $cgpaFormat = strtoupper($fetch_degree_array[$excel_row][15]);
                    }else{
                        if(is_float($fetch_degree_array[$excel_row][6])){
   ...
   public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id)
{
    ...
    $key = strtoupper(md5($serial_no.$dt)); //Updated by Mandar on 11-02-2022   md5($serial_no) => md5($serial_no.$dt)
    $codeContents = $key;
    $fileName = $key.'.png'; 
