...
class PdfGenerateBiharEcourtFeeTestJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
       ...
       // $krutidev = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\KRDV010.TTF', 'TrueTypeUnicode', '', 96);
        ...
        // foreach ($studentDataOrg as $studentData) {
            ...
            // }
                ...
                {
                    $bgPath="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/estamp/backend/canvas/bg_pdfs/bd/EStampingBihar_Rs_2_17Ups.pdf";//EStampingBihar_Rs_2_28_03_2023_test.pdf"; //EStampingBihar_Rs_2_".count($studentDataOrg['data'])."Ups.pdf";
                    foreach($studentDataOrg['data'] as $studentData) 
                    {
                        ...
                        $html = '<span style="font-family:dejavusans;">&#8377;'.$studentData['amount'].'</span>';
                        $pdfBig->setXY($stampX+27, $stampY+8.7);
                        $pdfBig->MultiCell(60, 0, $html, '', 'L', 0, 1, '', '', true, 0, true);

                        $pdfBig->SetFont($timesNewRoman, '', 7, '', false);
                    ...
                    $bgPath="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/estamp/backend/canvas/bg_pdfs/ws/EStampingBihar_Rs_2_1WS.pdf";
                    foreach($studentDataOrg['data'] as $studentData) 
                    {
                        if($i%3==1){
                            $stampX = 7.9;
                        ...
                        $pdfBig->SetFont($timesNewRoman, 'B', 7, '', false);
                        $pdfBig->setXY($stampX+4.1, $stampY+5.5);
                        $pdfBig->MultiCell(5, 0, $studentData['amount'], 0, 'C', 0, 1);

                        $pdfBig->setXY($stampX+52.6, $stampY+5.5);
                        $pdfBig->MultiCell(5, 0, $studentData['amount'], 0, 'C', 0, 1);

                        $pdfBig->SetFont($shreeDev, 'B', 6.5, '', false);

                        $pdfBig->setXY($stampX + 5, $stampY+20.5);
                        $pdfBig->MultiCell(30, 0, 'nƒrg ên`o', '', 'C', 0, 1, '', '', true, 0, true);//$studentData['amount_hi']

                        $pdfBig->SetFont($timesNewRoman, 'B', 5.5, '', false);
                        $pdfBig->setXY($stampX + 28, $stampY+20.5);
                        ...
                        $pdfBig->SetFont($timesNewRoman, '', 6, '', false);
