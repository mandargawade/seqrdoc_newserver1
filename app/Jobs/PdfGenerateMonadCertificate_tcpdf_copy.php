...
class PdfGenerateMonadCertificate
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $ghostImgArr = array();
        $pdfBig = new TCPDF('P', 'mm', array('250', '350'), true, 'UTF-8', false);
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			if($previewPdf==1){
				if($previewWithoutBg!=1){
					$pdfBig->Image($template_img_generate, 0, 0, '250', '350', "JPG", '', 'R', true);
			...
			$pdf = new TCPDF('P', 'mm', array('250', '350'), true, 'UTF-8', false); /* Verification PDF */
			...
			$pdf->AddPage();
			if($previewPdf!=1){
				$pdf->Image($template_img_generate, 0, 0, '250', '350', "JPG", '', 'R', true);
			}
			$pdf->setPageMark();			
			...
			$pdfBig->SetXY(15.5, 18);
			$pdfBig->MultiCell(0, 0, 'Roll No. '.$Roll_No, 0, 'L', 0, 0);	
			
			$pdfBig->SetFont($calibri,0, 12, '', false);
			$pdfBig->SetXY(190.5, 18);
			$pdfBig->MultiCell(0, 0, 'Serial No. '.$Serial_No, 0, 'L', 0, 0, '', '', true, 0, true);
			
			$pdf->SetFont($calibri, '', 12, '', false);
			$pdf->SetXY(15.5, 18);
			$pdf->MultiCell(0, 0, 'Roll No. '.$Roll_No, 0, 'L', 0, 0);	
			
			$pdf->SetFont($calibri,0, 12, '', false);
			$pdf->SetXY(190.5, 18);
			$pdf->MultiCell(0, 0, 'Serial No. '.$Serial_No, 0, 'L', 0, 0, '', '', true, 0, true);
			...
			$logoWidth = 35;
			$logoHeight = 28;
			$pdfBig->image($uv_logo,$logox,$logoy,$logoWidth,$logoHeight,"",'','L',true,3600); 
			$pdfBig->setPageMark();
			//$pdf->image($uv_logo,$logox,$logoy,$logoWidth,$logoHeight,"",'','L',true,3600); 
			//$pdf->setPageMark();		
			...
			$nameWidth = 32;
			$nameHeight = 20;
			$pdfBig->image($uv_name,$namex,$namey,$nameWidth,$nameHeight,"",'','L',true,3600); 
			$pdfBig->setPageMark();
			//$pdf->image($uv_name,$namex,$namey,$nameWidth,$nameHeight,"",'','L',true,3600); 
			//$pdf->setPageMark();
				
            //Start pdfBig			
            //Static Title        
            // start invisible data
            //$pdfBig->SetFont($timesb, '', 11, '', false); 
            //$pdfBig->SetTextColor(255, 255, 0);
            //$pdfBig->SetXY(18, 130);
			//$pdfBig->MultiCell(261, 0, $candidate_name, 0, 'L', 0, 0);
            // end invisible data 
			
			$pdfBig->SetFont($calibri, '', 14, '', false);
			$pdfBig->SetXY(20.5, 97);
			$pdfBig->MultiCell(210, 0, 'Enrollment No. '.$Enrollment_No, 0, 'C', 0, 0);			
			/*Hindi*/
			$pdfBig->SetFont($K026, '', 18, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(20.5, 159);
			$pdfBig->MultiCell(42, 0, 'izekf.kr fd;k tkrk gS fd', 0, 'L', 0, 0);
			$pdfBig->SetFont($K011, '', 24, '', false);
			$pdfBig->SetXY(60, 151);
			$pdfBig->setCellPaddings( $left = '', $top = '3.2', $right = '', $bottom = '');
			$pdfBig->MultiCell(170, 0, '<span style="color:#34398a;">'.$hindi_candidate_name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdfBig->SetFont($K026, '', 18, '', false);
            $pdfBig->SetXY(20.5, 169.7);
			$pdfBig->MultiCell(42, 0, 'vkRet @ vkRetk Jh-', 0, 'L', 0, 0);
			$pdfBig->SetFont($K011, '', 18, '', false);
			$pdfBig->SetXY(57, 165);
			$pdfBig->setCellPaddings( $left = '', $top = '2.4', $right = '', $bottom = '');
			$pdfBig->MultiCell(81, 0, '<span style="color:#34398a;">'.$Hindi_Father_Name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdfBig->SetFont($K026, '', 18, '', false);
            $pdfBig->SetXY(137, 169.7);
			$pdfBig->MultiCell(20, 0, ',oa Jherh-', 0, 'L', 0, 0);
			$pdfBig->SetFont($K011, '', 18, '', false);
			$pdfBig->SetXY(155, 165);
			$pdfBig->setCellPaddings( $left = '', $top = '2.4', $right = '', $bottom = '');
			$pdfBig->MultiCell(75, 0, '<span style="color:#34398a;">'.$Hindi_Mother_Name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');			
			$pdfBig->SetFont($K026, '', 18, '', false);
            $pdfBig->SetXY(20.5, 180.5);
			$pdfBig->MultiCell(45, 0, 'dks bl fo’ofo|ky; ls lu~', 0, 'L', 0, 0);
			$pdfBig->SetFont($K011, '', 18, '', false);
			$pdfBig->SetXY(63, 175);
			$pdfBig->setCellPaddings( $left = '', $top = '2.7', $right = '', $bottom = '');
			$pdfBig->MultiCell(43, 0, '<span style="color:#34398a;">'.$Hindi_Exam_Year.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdfBig->SetFont($K026, '', 18, '', false);
            $pdfBig->SetXY(108, 180.5);
			$pdfBig->MultiCell(6, 0, 'dh', 0, 'L', 0, 0);
			$pdfBig->SetFont($K011, '', 18, '', false);
			$pdfBig->SetXY(113, 175);
			$pdfBig->setCellPaddings( $left = '', $top = '2.7', $right = '', $bottom = '');
			$pdfBig->MultiCell(117, 0, '<span style="color:#34398a;">'.$Hindi_Degree_In.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdfBig->SetFont($K026, '', 18, '', false);
            $pdfBig->SetXY(20.5, 190);
			$pdfBig->MultiCell(30, 0, 'ikB~îdze ijh{kk esa', 0, 'L', 0, 0);
			$pdfBig->SetFont($K011, '', 18, '', false);
			$pdfBig->SetXY(48.5, 184.5);
			$pdfBig->setCellPaddings( $left = '', $top = '2.7', $right = '', $bottom = '');
			$pdfBig->MultiCell(155, 0, '<span style="color:#34398a;">'.$Hindi_Degree.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdfBig->SetFont($K026, '', 18, '', false);
            $pdfBig->SetXY(204, 190);
			$pdfBig->MultiCell(27, 0, 'dh lun@mikf/k', 0, 'L', 0, 0);
			$pdfBig->SetFont($K011, '', 18, '', false);
			$pdfBig->SetXY(21, 195);
			$pdfBig->setCellPaddings( $left = '', $top = '2.9', $right = '', $bottom = '');
			$pdfBig->MultiCell(169, 0, '<span style="color:#34398a;">'.$Hindi_Remark.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdfBig->SetFont($K026, '', 18, '', false);
            $pdfBig->SetXY(191, 200);
			$pdfBig->MultiCell(40, 0, 'Js.kh esa iznÙk dh x;h gSA', 0, 'L', 0, 0);
			
			
			/*English*/
			$pdfBig->SetFont($timesi, '', 15, '', false);
            $pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(20.5, 224);
			$pdfBig->MultiCell(0, 0, 'This is to certify that', 0, 'L', 0, 0);			
			$pdfBig->SetFont($timesb, '', 20, '', false);
			$pdfBig->SetXY(66, 220);
			$pdfBig->MultiCell(132, 0, '<span style="color:#34398a;">'.$candidate_name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);			
			$pdfBig->SetFont($timesi, '', 15, '', false);
			$pdfBig->SetXY(198, 224);
			$pdfBig->MultiCell(33, 0, 'Son/Daughter', 0, 'L', 0, 0);
			$pdfBig->SetFont($timesi, '', 15, '', false);
            $pdfBig->SetXY(20.5, 234);
			$pdfBig->MultiCell(20, 0, 'of Shri.', 0, 'L', 0, 0);
			$pdfBig->SetFont($timesb, '', 16, '', false);
			$pdfBig->SetXY(37, 231.5);
			$pdfBig->MultiCell(104, 0, '<span style="color:#34398a;">'.$Father_Name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($timesi, '', 15, '', false);
            $pdfBig->SetXY(142, 234);
			$pdfBig->MultiCell(22, 0, 'and Smt.', 0, 'L', 0, 0);
			$pdfBig->SetFont($timesb, '', 16, '', false);
			$pdfBig->SetXY(161, 231.5);
			$pdfBig->MultiCell(69, 0, '<span style="color:#34398a;">'.$Mother_Name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($timesi, '', 15, '', false);
            $pdfBig->SetXY(20.5, 244.5);
			$pdfBig->MultiCell(93, 0, 'has been conferred the Diploma/Degree of', 0, 'L', 0, 0);
			$pdfBig->SetFont($timesb, '', 16, '', false);
			$pdfBig->SetXY(112, 242);
			$pdfBig->MultiCell(118, 0, '<span style="color:#34398a;">'.$Degree.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($timesi, '', 15, '', false);
            $pdfBig->SetXY(20.5, 255);
			$pdfBig->MultiCell(14, 0, 'in the', 0, 'L', 0, 0);
			$pdfBig->SetFont($timesb, '', 16, '', false);
			$pdfBig->SetXY(34, 252.5);
			$pdfBig->MultiCell(92, 0, '<span style="color:#34398a;">'.$Degree_In.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($timesi, '', 15, '', false);
            $pdfBig->SetXY(126, 255);
			$pdfBig->MultiCell(50, 0, 'course examination of', 0, 'L', 0, 0);
			$pdfBig->SetFont($timesb, '', 16, '', false);
			$pdfBig->SetXY(174, 252.5);
			$pdfBig->MultiCell(39, 0, '<span style="color:#34398a;">'.$Exam_Year.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($timesi, '', 15, '', false);
            $pdfBig->SetXY(214, 255);
			$pdfBig->MultiCell(16, 0, 'of this', 0, 'L', 0, 0);
			$pdfBig->SetFont($timesi, '', 15, '', false);
            $pdfBig->SetXY(20.5, 266);
			$pdfBig->MultiCell(30, 0, 'University in', 0, 'L', 0, 0);
			$pdfBig->SetFont($timesb, '', 16, '', false);
			$pdfBig->SetXY(49, 264);
			$pdfBig->MultiCell(160, 0, '<span style="color:#34398a;">'.$Remark.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($timesi, '', 15, '', false);
            $pdfBig->SetXY(209, 266);
			$pdfBig->MultiCell(28, 0, 'Division.', 0, 'L', 0, 0);
			
			$pdfBig->SetFont($times, '', 17.5, '', false);	
			$pdfBig->SetXY(15, 312);
			$pdfBig->MultiCell(73.33333333333333, 0, 'Registrar',0, 'C', 0, 0);
			$pdfBig->MultiCell(73.33333333333333, 0, 'Date: '.$Date,0, 'C', 0, 0);
			$pdfBig->MultiCell(73.33333333333333, 0, 'Vice-Chancellor',0, 'C', 0, 0);
			...
			/*Hindi*/
			$pdf->SetFont($K026, '', 18, '', false);
            $pdf->SetTextColor(0, 0, 0);
			$pdf->SetXY(20.5, 159);
			$pdf->MultiCell(42, 0, 'izekf.kr fd;k tkrk gS fd', 0, 'L', 0, 0);
			$pdf->SetFont($K011, '', 24, '', false);
			$pdf->SetXY(60, 151);
			$pdf->setCellPaddings( $left = '', $top = '3.2', $right = '', $bottom = '');
			$pdf->MultiCell(170, 0, '<span style="color:#34398a;">'.$hindi_candidate_name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdf->SetFont($K026, '', 18, '', false);
            $pdf->SetXY(20.5, 169.7);
			$pdf->MultiCell(42, 0, 'vkRet @ vkRetk Jh-', 0, 'L', 0, 0);
			$pdf->SetFont($K011, '', 18, '', false);
			$pdf->SetXY(57, 165);
			$pdf->setCellPaddings( $left = '', $top = '2.4', $right = '', $bottom = '');
			$pdf->MultiCell(81, 0, '<span style="color:#34398a;">'.$Hindi_Father_Name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdf->SetFont($K026, '', 18, '', false);
            $pdf->SetXY(137, 169.7);
			$pdf->MultiCell(20, 0, ',oa Jherh-', 0, 'L', 0, 0);
			$pdf->SetFont($K011, '', 18, '', false);
			$pdf->SetXY(155, 165);
			$pdf->setCellPaddings( $left = '', $top = '2.4', $right = '', $bottom = '');
			$pdf->MultiCell(75, 0, '<span style="color:#34398a;">'.$Hindi_Mother_Name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');			
			$pdf->SetFont($K026, '', 18, '', false);
            $pdf->SetXY(20.5, 180.5);
			$pdf->MultiCell(45, 0, 'dks bl fo’ofo|ky; ls lu~', 0, 'L', 0, 0);
			$pdf->SetFont($K011, '', 18, '', false);
			$pdf->SetXY(63, 175);
			$pdf->setCellPaddings( $left = '', $top = '2.7', $right = '', $bottom = '');
			$pdf->MultiCell(43, 0, '<span style="color:#34398a;">'.$Hindi_Exam_Year.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdf->SetFont($K026, '', 18, '', false);
            $pdf->SetXY(108, 180.5);
			$pdf->MultiCell(6, 0, 'dh', 0, 'L', 0, 0);
			$pdf->SetFont($K011, '', 18, '', false);
			$pdf->SetXY(113, 175);
			$pdf->setCellPaddings( $left = '', $top = '2.7', $right = '', $bottom = '');
			$pdf->MultiCell(117, 0, '<span style="color:#34398a;">'.$Hindi_Degree_In.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdf->SetFont($K026, '', 18, '', false);
            $pdf->SetXY(20.5, 190);
			$pdf->MultiCell(30, 0, 'ikB~îdze ijh{kk esa', 0, 'L', 0, 0);
			$pdf->SetFont($K011, '', 18, '', false);
			$pdf->SetXY(48.5, 184.5);
			$pdf->setCellPaddings( $left = '', $top = '2.7', $right = '', $bottom = '');
			$pdf->MultiCell(155, 0, '<span style="color:#34398a;">'.$Hindi_Degree.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdf->SetFont($K026, '', 18, '', false);
            $pdf->SetXY(204, 190);
			$pdf->MultiCell(27, 0, 'dh lun@mikf/k', 0, 'L', 0, 0);
			$pdf->SetFont($K011, '', 18, '', false);
			$pdf->SetXY(21, 195);
			$pdf->setCellPaddings( $left = '', $top = '2.9', $right = '', $bottom = '');
			$pdf->MultiCell(169, 0, '<span style="color:#34398a;">'.$Hindi_Remark.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			$pdf->SetFont($K026, '', 18, '', false);
            $pdf->SetXY(191, 200);
			$pdf->MultiCell(40, 0, 'Js.kh esa iznÙk dh x;h gSA', 0, 'L', 0, 0);
			
			
			/*English*/
			$pdf->SetFont($timesi, '', 15, '', false);
            $pdf->SetTextColor(0, 0, 0);
			$pdf->SetXY(20.5, 224);
			$pdf->MultiCell(0, 0, 'This is to certify that', 0, 'L', 0, 0);			
			$pdf->SetFont($timesb, '', 20, '', false);
			$pdf->SetXY(66, 220);
			$pdf->MultiCell(132, 0, '<span style="color:#34398a;">'.$candidate_name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);			
			$pdf->SetFont($timesi, '', 15, '', false);
			$pdf->SetXY(198, 224);
			...
			$pdf->SetFont($timesb, '', 16, '', false);
			$pdf->SetXY(37, 231.5);
			$pdf->MultiCell(104, 0, '<span style="color:#34398a;">'.$Father_Name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($timesi, '', 15, '', false);
            $pdf->SetXY(142, 234);
			$pdf->MultiCell(22, 0, 'and Smt.', 0, 'L', 0, 0);
			$pdf->SetFont($timesb, '', 16, '', false);
			$pdf->SetXY(161, 231.5);
			$pdf->MultiCell(69, 0, '<span style="color:#34398a;">'.$Mother_Name.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($timesi, '', 15, '', false);
            $pdf->SetXY(20.5, 244.5);
			$pdf->MultiCell(93, 0, 'has been conferred the Diploma/Degree of', 0, 'L', 0, 0);
			$pdf->SetFont($timesb, '', 16, '', false);
			$pdf->SetXY(112, 242);
			$pdf->MultiCell(118, 0, '<span style="color:#34398a;">'.$Degree.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($timesi, '', 15, '', false);
            $pdf->SetXY(20.5, 255);
			$pdf->MultiCell(14, 0, 'in the', 0, 'L', 0, 0);
			$pdf->SetFont($timesb, '', 16, '', false);
			$pdf->SetXY(34, 252.5);
			$pdf->MultiCell(92, 0, '<span style="color:#34398a;">'.$Degree_In.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($timesi, '', 15, '', false);
            $pdf->SetXY(126, 255);
			$pdf->MultiCell(50, 0, 'course examination of', 0, 'L', 0, 0);
			$pdf->SetFont($timesb, '', 16, '', false);
			$pdf->SetXY(174, 252.5);
			$pdf->MultiCell(39, 0, '<span style="color:#34398a;">'.$Exam_Year.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($timesi, '', 15, '', false);
            $pdf->SetXY(214, 255);
			...
			$pdf->SetFont($timesb, '', 16, '', false);
			$pdf->SetXY(49, 264);
			$pdf->MultiCell(160, 0, '<span style="color:#34398a;">'.$Remark.'</span>', $cell_border, 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($timesi, '', 15, '', false);
            $pdf->SetXY(209, 266);
			$pdf->MultiCell(28, 0, 'Division.', 0, 'L', 0, 0);
			
			$pdf->SetFont($times, '', 17.5, '', false);	
			$pdf->SetXY(15, 312);
			$pdf->MultiCell(73.33333333333333, 0, 'Registrar',0, 'C', 0, 0);
			$pdf->MultiCell(73.33333333333333, 0, 'Date: '.$Date,0, 'C', 0, 0);
			$pdf->MultiCell(73.33333333333333, 0, 'Vice-Chancellor',0, 'C', 0, 0);			
