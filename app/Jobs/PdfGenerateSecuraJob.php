...
class PdfGenerateSecuraJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {            
            ...
            if($previewPdf==1){
                if($previewWithoutBg!=1){
                    $pdfBig->Image($template_img_generate, 0, 0, 296.926,  210.058, 'jpg', '', true, false);
                }
                $date_font_size = '11';
                $date_nox = 13;
                $date_noy = 10;
            ...
            $av_url=$studentData[7];
            $photo=$studentData[8];            
            $serial_no='C_'.$studentData[0];
            $GUID=$studentData[0];
            ...
            if($previewPdf!=1){
                ...
                $pdf->output($myPath . DIRECTORY_SEPARATOR . 'C_'.$certName, 'F');
                $this->addCertificate($serial_no, $certName, $dtc, $template_id, $admin_id);
        ...
        $file_name =  str_replace("/", "_",'molwaCertificate'.date("Ymdhms")).'.pdf';
        ...
        if($previewPdf!=1){
            ...
            $template_name="molwaCertificate";
...
file_put_contents($local_server_file_path, file_get_contents($destinationURL));
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id)
    {
        ...
        $file1 = public_path().'/backend/temp_pdf_file/C_'.$certName;
        $file2 = public_path().'/backend/pdf_file/C_'.$certName;
        ...
        $pdfActualPath=public_path().'/'.$subdomain[0].'/backend/pdf_file/C_'.$certName;
        ...
        $result = SbStudentTable::create(['serial_no'=>$serial_no,'certificate_filename'=>'C_'.$certName,'template_name'=>$template_id,'key'=>$key,'path'=>$urlRelativeFilePath,'created_at'=>$datetime,'created_by'=>$ses_id,'updated_at'=>$datetime,'updated_by'=>$ses_id,'status'=>$sts,'site_id'=>$auth_site_id]);
        }else{
        $resultu = StudentTable::where('serial_no',$serial_no)->update(['status'=>'0']);
        // Insert the new record
        
        $result = StudentTable::create(['serial_no'=>$serial_no,'certificate_filename'=>'C_'.$certName,'template_name'=>$template_id,'key'=>$key,'path'=>$urlRelativeFilePath,'created_at'=>$datetime,'created_by'=>$ses_id,'updated_at'=>$datetime,'updated_by'=>$ses_id,'status'=>$sts,'site_id'=>$auth_site_id,'template_type'=>2]);
        }
