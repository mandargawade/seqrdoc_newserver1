...
class ExcelGenerateJob implements ShouldQueue
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $fonts_array = $pdf_data['fonts_array'];
        $rowData = $pdf_data['rowData'];
        $log_serial_no = $pdf_data['log_serial_no'];
        $excel_row = $pdf_data['excel_row'];
        \Log::info($excel_row);
            ...
            if(!$isPreview) {
                ...
                $fp = fopen(public_path()."/backend/canvas/dummy_images/".$template_name.'/'.$withoutExt.".txt","a");
                fwrite($fp,$content);
                fwrite($fp,$date);
