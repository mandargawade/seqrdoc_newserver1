...
class PdfGenerateUNEBJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $universe = TCPDF_FONTS::addTTFfont(public_path().'\\backend\canvas\fonts\Univers 55 Regular.ttf', 'TrueTypeUnicode', '', 96);
        $universeBold = TCPDF_FONTS::addTTFfont(public_path().'\\backend\canvas\fonts\UNIVERS 55 BOLD.TTF', 'TrueTypeUnicode', '', 96);
		...
		$preview_serial_no=1;
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('UNEBCert');
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            $high_res_bg="uneb_blank.jpg"; 
            $low_res_bg="uneb_blank.jpg";
            $pdfBig->AddPage();
            ...
            $centreNo = $studentData[1];
            $centreName = $studentData[2];
            ...
            $result = $studentData[9];
            $aggregate = $studentData[10];
            
            $date = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($studentData[31])->format('jS F Y');
            ...
            $pdfBig->SetFont($courierNewBold, '', 11, '', false);
            $pdfBig->setXY(145, 47);
            $pdfBig->MultiCell(48, 0, $date, '', 'L', 0, 1);
            ...
            $pdfBig->SetFont($universe, '', 12.7, '', false);
            $text = 'This is ro certify that the candidate named below sat <span style="font-family:'.$universeBold.';">Uganda Certificate of Education/Uganda Advanced Certificate of Education</span> Examinations and obtained the following results:-';
            $pdfBig->setXY(17, 87);
            ...
            for($i=1;$i<=10;$i++)
            {
                ...
                if($subject != "")
                {
                    $pdfBig->SetFont($universe, '', 10, '', false);
            ...
            $pdfBig->MultiCell(15, 0, $result, '', 'C', 0, 1);
            ...
            $pdf->SetFont($courierNewBold, '', 11, '', false);
            $pdf->setXY(172, 207);
            $pdf->MultiCell(15, 0, $result, '', 'C', 0, 1);
            ...
            if($previewPdf!=1){
                ...
                $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'UNEBCert',$admin_id,$card_serial_no);
       ...
       if($previewPdf!=1){
        $this->updateCardNo('UNEBCert',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'UNEBCert'.date("Ymdhms")).'.pdf';
