...
class PdfGenerateRRMUJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) 
        {
            ...
            $pdf->SetFont($times, '', 17, '', false);
            $pdf->SetXY($x, $y);
            $pdf->Cell(0, 0, "having passed the University Examination held in the year ".$year." with", 0, false, 'C');

            $pdfBig->SetTextColor(0,0,0);  
            $pdfBig->SetFont($times, '', 17, '', false);
            $pdfBig->SetXY($x, $y);
            $pdfBig->Cell(0, 0, "having passed the University Examination held in the year ".$year." with", 0, false, 'C');
            ...
            $studentData[6]=ucwords(strtolower($studentData[6]));
            $studentData[6] = preg_replace(array('/\bOf\b/','/\bIs\b/','/\bAnd\b/','/\bIn\b/','/\bOn\b/'),array('of','is','and','in','on'),$studentData[6]);
            ...
            $studentData[7]=ucwords(strtolower($studentData[7]));
            $studentData[7] = preg_replace(array('/\bOf\b/','/\bIs\b/','/\bAnd\b/','/\bIn\b/','/\bOn\b/'),array('of','is','and','in','on'),$studentData[7]);
            ...
            $x = 10;
            $y = 188;
            $pdf->SetTextColor(237, 50, 55);//$studentData[8]   
            $pdf->SetFont($K101, '', 28, '', false);//Kruti_Dev_730k
            $pdf->SetXY($x, $y);
            $pdf->Cell(0, 0, $studentData[8], 0, false, 'C');

            $pdfBig->SetTextColor(237, 50, 55);   
            $pdfBig->SetFont($K101, '', 28, '', false);
            $pdfBig->SetXY($x, $y);
            ...
            $pdf->SetXY($x, $y);
            $pdf->Cell(0, 0, " ".$year." ", 0, false, 'L');

            $pdfBig->SetFont($Arial, '', 17, '', false);
            $pdfBig->SetXY($x, $y);
            $pdfBig->Cell(0, 0, " ".$year." ", 0, false, 'L');
            ...
            $y = 264;
            $pdf->SetTextColor(0,0,0);   
            $pdf->SetFont($K101, '', 12, '', false);
            $pdf->SetXY($x, $y);
            $pdf->Cell(0, 0, "dqyifr", 0, false, 'L');

            $pdfBig->SetTextColor(0,0,0);   
            $pdfBig->SetFont($K101, '', 12, '', false);
            $pdfBig->SetXY($x, $y);
            $pdfBig->Cell(0, 0, "dqyifr", 0, false, 'L');
            ...
            $str= $studentData[1].", ".$studentData[4].", ".$studentData[6]." ".$studentData[7]." , ".$studentData[12].", ".$studentData[5]." ".$statictext.", ".$studentData[13]."";
            $codeContents = $str;
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id)
    {
        ...
        $awsFolderCheck = 'public/'.$subdomain[0].'/backend/pdf_file/Inactive_PDF';

        $outputFileExist = 'public/'.$subdomain[0]."\\backend\\pdf_file\\".$certName;

        $movedfolder = 'public/'.$subdomain[0]."/backend/pdf_file";

        $s3 = \Storage::disk('s3');
        // // File Exist in folder
        if($s3->exists($outputFileExist)) {
            $student = StudentTable::where('status',1)->where('serial_no',$serial_no)->value('id');
            
            // check folder 
            if($s3->exists($awsFolderCheck)) {
                $newFileNamePdf = 'public/'.$subdomain[0].'/backend/pdf_file/Inactive_PDF/'.$student.'_'.$certName;
                $s3->move($outputFileExist, $newFileNamePdf);
            } else {
                // folder create
                $s3->makeDirectory($awsFolderCheck, 0777);
                $newFileNamePdf = 'public/'.$subdomain[0].'/backend/pdf_file/Inactive_PDF/'.$student.'_'.$certName;
                $s3->move($outputFileExist, $newFileNamePdf);
            }
        }
        if(!$s3->exists($outputFileExist)) {
            $s3->put($outputFileExist, file_get_contents($output));
            //$s3->put($outputFileExist, file_get_contents($outputFileExist));
        }
        //echo $outputFileExist;
        //echo "\n";
        //echo $output;
        @unlink($output);
        // rohit changes 18/05/2023
