...
class PdfGenerateBiharEcourtFeeJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $TimesNewRoman_B = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\TimesNewRoman_B.ttf', 'TrueTypeUnicode', '', 96);
        ...
        $krutidev = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\KRUTI-DEV-630.TTF', 'TrueTypeUnicode', '', 96);
        $shreeDev = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\S1097890.TTF', 'TrueTypeUnicode', '', 96);
        $RUPEE_FORADIAN = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\RUPEE_FORADIAN.TTF', 'TrueTypeUnicode', '', 96);
        //$card_serial_no="";
        $preview_serial_no=1;
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('BiharEcourtFeeCert');
        ...
        // foreach ($studentDataOrg as $studentData) {
            ...
            // }
                ...
                $serial_no=$GUID=$studentDataOrg['transaction_id'];
                $dt = date("_ymdHis");
                ...
                $style1D = array(
                    ...
                    'text' => false, // was true in live system. changed by bhavin.
                    'font' => 'calibri',
                    'fontsize' => 1,
                    'stretchtext' => 4
                );    
                ...
                $stamp_type=strtolower($studentDataOrg['stamp_type']);
                $i = 1;
                if(strtolower($studentDataOrg['stamp_type']) == "bd")
                {
                    $countData=count($studentDataOrg['data']);
                    $bgPath="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/estamp/backend/canvas/bg_pdfs/bd/EStampingBihar_Rs_2_".$countData."Ups.pdf";
                    foreach($studentDataOrg['data'] as $studentData) 
                    {
                        if($i%3==1){
                            $stampX = 7.8;
                        }elseif($i % 3 == 2){
                        ...
                        // $template_img_generate = public_path().'\\'.$subdomain[0].'\backend\canvas\bg_images\walfarestamp.jpg';   
                        // $pdfBig->setPageMark(); 
                        // $pdfBig->Image($template_img_generate, $stampX, $stampY, '62.5', '35.5', "JPG", '', 'R', true);
                        // $pdfBig->setPageMark(); 

                        $template_stamp_img_generate = public_path().'\\'.$subdomain[0].'\backend\canvas\bg_images\stamp.png';   
                        $pdfBig->setPageMark(); 
                        $pdfBig->Image($template_stamp_img_generate, $stampX+22.5, $stampY+8.5, '5', '7', "PNG", '', 'C', false);
                        $pdfBig->setPageMark(); 

                        /*$pdfBig->SetFont($timesNewRoman, '', 15, '', false);
                        $html = '<span style="font-family:dejavusans;">&#8377;</span>'.$studentData['amount'];
                        $pdfBig->setXY($stampX+27, $stampY+8.7);
                        $pdfBig->MultiCell(60, 0, $html, '', 'L', 0, 1, '', '', true, 0, true);*/

                        $pdfBig->SetFont($RUPEE_FORADIAN, '', 15, '', false);
                        $pdfBig->setXY($stampX, $stampY+8.7);
                        $pdfBig->MultiCell(60, 0, '`', '', 'C', 0, 1, '', '', true, 0, true);

                        $pdfBig->SetFont($TimesNewRoman_B, 'B', 15, '', false);
                        //$html = '<span style="font-family:dejavusans;">&#8377;</span>'.$studentData['amount'];
                         // $html = '<span style="font-family:RUPEE_FORADIAN;">`</span>'.$studentData['amount'];

                        $pdfBig->setXY($stampX+29.8, $stampY+8.5);
                        $pdfBig->MultiCell(60, 0, $studentData['amount'], '', 'L', 0, 1, '', '', true, 0, true);

                        //Bhavin commented this as we moved the barcode upwards.
                        /*
                        $pdfBig->SetFont($TimesNewRoman_B, 'B', 8, '', false);
                        //if($countData==21){
                            $pdfBig->setXY($stampX, $stampY+16);
                        $pdfBig->MultiCell(60, 0, $studentData['certificateno'], '', 'C', 0, 1, '', '', true, 0, true);
                        $pdfBig->setXY($stampX, $stampY+20);
                        $pdfBig->MultiCell(60, 0, $studentData['date'], '', 'C', 0, 1, '', '', true, 0, true);

                         $codeContents = $studentData['certificateno'];
                        $qrCodex = $stampX+12.5;
                        $qrCodey = $stampY+25;
                        $qrCodeWidth =38;
                        $qrCodeHeight = 7;
                        
                        $pdfBig->write1DBarcode($codeContents, 'C128A', $qrCodex, $qrCodey, $qrCodeWidth, $qrCodeHeight,0.4, $style1D, 'N');
                        */

                        // Bhavin moved barcode up and moved number and date below 12-05-2023
                        // Changes made only in X and Y co-ordinates
                        $pdfBig->SetFont($TimesNewRoman_B, 'B', 8, '', false);
                        $pdfBig->setXY($stampX, $stampY+25);//18
                        $pdfBig->MultiCell(60, 0, $studentData['certificateno'], '', 'C', 0, 1, '', '', true, 0, true);
                        $pdfBig->setXY($stampX, $stampY+29);//22
                        $pdfBig->MultiCell(60, 0, $studentData['date'], '', 'C', 0, 1, '', '', true, 0, true);

                        $codeContents = $studentData['certificateno'];
                        $qrCodex = $stampX+6.5;
                        $qrCodey = $stampY+17;//27
                        $qrCodeWidth =49;
                        $qrCodeHeight = 7;
                        
                        //$style1D replaced by null on 24-05-2023 by Mandar
                        $pdfBig->write1DBarcode($codeContents, 'C128A', $qrCodex, $qrCodey, $qrCodeWidth, $qrCodeHeight,0.4,null , 'N');


                       /* }else{
                            $pdfBig->setXY($stampX, $stampY+18);
                        $pdfBig->MultiCell(60, 0, $studentData['certificateno'], '', 'C', 0, 1, '', '', true, 0, true);
                        $pdfBig->setXY($stampX, $stampY+22);
                        $pdfBig->MultiCell(60, 0, $studentData['date'], '', 'C', 0, 1, '', '', true, 0, true);
                ...
                }elseif(strtolower($studentDataOrg['stamp_type']) == "ws"){
                    $bgPath="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/estamp/backend/canvas/bg_pdfs/ws/WelfareStamp_".count($studentDataOrg['data']).".pdf";
                    foreach($studentDataOrg['data'] as $studentData) 
                    {
                        if($i%3==1){
                            $stampX = 7.7;
                        ...
                        // $template_img_generate = public_path().'\\'.$subdomain[0].'\backend\canvas\bg_images\wsstamp.jpg';   
                        // $pdfBig->setPageMark(); 
                        // $pdfBig->Image($template_img_generate, $stampX, $stampY, '62.5', '35.5', "JPG", '', 'R', true);
                        // $pdfBig->setPageMark(); 

                        $pdfBig->SetFont($krutidev, 'B', 8, '', false);
                        $pdfBig->setXY($stampX+4.3, $stampY+5);
                        $pdfBig->MultiCell(5, 0, $studentData['amount'], 0, 'C', 0, 1);

                        $pdfBig->SetFont($timesNewRoman, 'B', 8, '', false);
                        $pdfBig->setXY($stampX+52.8, $stampY+5);
                        $pdfBig->MultiCell(5, 0, $studentData['amount'], 0, 'C', 0, 1);

                        $pdfBig->SetFont($shreeDev, 'B', 9, '', false);

                        $pdfBig->setXY($stampX + 5, $stampY+20.1);
                        $pdfBig->MultiCell(30, 0, 'nƒrg ên`o', '', 'C', 0, 1, '', '', true, 0, true);//$studentData['amount_hi']

                        $pdfBig->SetFont('Arial', 'B', 6, '', false);
                        $pdfBig->setXY($stampX + 26, $stampY+20.3);
                        $pdfBig->MultiCell(30, 0, $studentData['amount_en'], '', 'L', 0, 1, '', '', true, 0, true);

                        $pdfBig->SetFont($timesNewRoman, 'B', 5.5, '', false);
                        $codeContents = $studentData['certificateno'];
                        $qrCodex = $stampX+5.5;
                        $qrCodey = $stampY + 28;
                        $qrCodeWidth =51;
                        ...
                        $pdfBig->SetFont($TimesNewRoman_B, '', 6, '', false);
                        $pdfBig->setXY($stampX+1, $stampY + 48);
                        ...
                        $pdfBig->setXY($stampX+58.8, $stampY + 47);
                        $pdfBig->StartTransform();
                        $pdfBig->Rotate(90);
                        // $pdfBig->setXY($stampX, $stampY + 38);
                        $pdfBig->MultiCell(60, 0, $studentData['date'], '', 'C', 0, 1, '', '', true, 0, true);
                        $pdfBig->Rotate(0);
                        $pdfBig->StopTransform();
            ...
            if($previewPdf!=1){
                ...
                $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'BiharEcourtFeeCert',$admin_id,$card_serial_no);
       ...
       if($previewPdf!=1){
        $this->updateCardNo('BiharEcourtFeeCert',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        //$file_name_pre =  str_replace("/", "_",'BiharEcourtFeeCert'.date("Ymdhms"));
        //$file_name =  str_replace("/", "_",'BiharEcourtFeeCert'.$printerName.date("Ymdhms")).'.pdf';
        
        $file_name =  str_replace("/", "_",$serial_no.'_'.$stamp_type).'.pdf';
