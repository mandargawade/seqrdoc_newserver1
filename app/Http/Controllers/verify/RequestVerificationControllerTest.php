...
class RequestVerificationControllerTest extends Controller
{
					...
					if ($trans_status == '1') {
						...
						if (!empty($verification_requests)) {
			               ...
			               /* if($subdomain[0]=="demo"){
							...
							if(($subdomain[0] == "raisoni"||$subdomain[0] == "demo")&&$user_email=="siddhi.scube@gmail.com"){
...
/*
	...
	public function testEmail(){
		...
		$domain = \Request::getHost();
	    $subdomain = explode('.', $domain);
					$trans_id_ref = $this->sanitizeVar('AAAA');
					$trans_id_gateway = $this->sanitizeVar('BBB');
					$payment_mode = $this->sanitizeVar('CCC');
					$amount = $this->sanitizeVar('DDD');
					$additional = $this->sanitizeVar('EEE');
					$user_id = $this->sanitizeVar('FFF');
					$student_key = $this->sanitizeVar('OCV-R-498');
					$trans_status = $this->sanitizeVar('1');
					$transaction_date = date('Y-m-d H:i:s');
