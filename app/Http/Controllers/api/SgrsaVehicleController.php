...
class SgrsaVehicleController extends Controller
{
    public function RecallScanViewCertificate(Request $request){
        ...
        { 
			...
			if(ApiSecurityLayer::checkAuthorization())	
            {
				...
				$vrn=DB::table('recall_informations')
					->select('encryptedString')
					->where(['vehicle_reg_no' => $vehicle_reg_no,'publish' => 1])
					->get();
				$key=$vrn[0]->encryptedString;
                $scan_data = [];
        ...
        {   
           ...
           if(!$response['status']){
            ...
            if (!empty($user_id) /*&& ApiSecurityLayer::checkAccessTokenInstitute($user_id)*/&& $user_id==$data['user_id']) 
            {
                ...
                $scanndHistory['scan_result'] = $scan_result;
                $scanndHistory['site_id'] = $site_id;
                if($subdomain[0]=="test"||$subdomain[0]=="sgrsa"){
