        ...
        if (ApiSecurityLayerThirdParty::checkAuthorization()){
            ...
            if($request['file'] != 'undefined'&&$request['file'] != ''){
                ...
                if($request['file']->move($target_path,$excelfile)){
                 ...
                 if((isset($request->OverWriteRepeat)&&$request->OverWriteRepeat!=1)||!isset($request->OverWriteRepeat)){   
            ...
            $dataArr=array("request_id"=>$request_id,"template_id"=>$template_id,"excel_file"=>$excelUrl,"generation_type"=>$request->generation_type,"status"=>"In Process","total_documents"=>$old_rows+$new_rows,"generated_documents"=>$new_rows,"regenerated_documents"=>$old_rows,"printable_pdf_link"=>"");

            $key=\Config::get('constant.encryption_key');
            $token=ApiSecurityLayerThirdParty::encrypt_adv($request_id, $key);
            $call_back_url = "http://".$domain."/api/seqrdoc-check-status/".$token;
            $updated=date('Y-m-d H:i:s');
            $message = array('status' => 200, 'message' => 'Generation started.',"call_back_url"=>$call_back_url,"data"=>$dataArr);
            ThirdPartyRequests::where('id',$request_id)->update(['call_back_url'=>$call_back_url,"updated_at"=>$updated,"response"=>json_encode($message)]);
            ...
            $insertRequestSeqr->call_back_url=$call_back_url;
