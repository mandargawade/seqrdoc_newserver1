...
use App\Jobs\PdfGenerateBiharEstampEcourtJob;
use App\Jobs\PdfGenerateBiharEcourtFeeJob;
...
*/
    ...
    public function generateEcourt(Request $request){
          ...
          isset($requestArr['Printer'])&&!empty($requestArr['Printer']))
        {
            ...
            $link = $this->dispatch(new PdfGenerateBiharEstampEcourtJob($pdfData));
            return response()->json(['status'=>200,'message'=>'Certificates generated successfully.','pdfUrl'=>$link]);
     ...
     public function generateCourtFee(){
          ...
          isset($requestArr['stamp_type'])&&!empty($requestArr['data']))
        {
            ...
            $link = $this->dispatch(new PdfGenerateBiharEcourtFeeJob($pdfData));
            return response()->json(['status'=>200,'message'=>'Certificates generated successfully.','pdfUrl'=>$link]);
       ...
       public function storeFileToLocal(Request $request){
        $domain = \Request::getHost();
        $subdomain = explode('.', $domain);
        ...
        $path=public_path().'/'.$subdomain[0].'/backend/tcpdf/downloaded/'.$fileName;
        ...
        {   
            ...
            $resp=['status'=>1,"message"=>"success", 'base64'=>$base64,"fileName"=>$fileName];
        }
        else
        {
     ...
     public function fileDelete(){
        ...
        if(isset($requestArr['fileName'])&&!empty($requestArr['fileName']))
        {
            $filePath=public_path().'\\'.$subdomain[0].'\backend\tcpdf\examples\\'.$requestArr['fileName'];
            if(file_exists($filePath)){
                if(unlink($filePath)){
                         return $message = array('status' => 200, 'message' => 'File deleted successfully.');
           
                }else{
                         return $message = array('status' => 400, 'message' => 'Something went wrong while deleting file.');
                }
            }else{
                 return $message = array('status' => 400, 'message' => 'File not found.');
           }
