...
use Helper;
use App\Jobs\ValidateExcelUNEBJob;
use App\Jobs\PdfGenerateUNEBJob;
use App\Jobs\PdfGenerateUNEBCOEJob;
use App\Jobs\PdfGenerateUNEBECOEJob;

class UNEBCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.uneb.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelUNEBJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
		...
		if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateUNEBJob($pdfData));
        }
        elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateUNEBCOEJob($pdfData));
        }elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateUNEBECOEJob($pdfData));
        }
