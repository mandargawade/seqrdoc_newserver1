...
use App\Jobs\ValidateExcelGalgotiasJob;
use App\Jobs\PdfGenerateGalgotiasJob;
...
class DegreeCertificateController extends Controller
{
    ...
    public function uploadpage(){
      ...
      return view('admin.galgotias.index');
    }
     ...
     public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
               ...
               $excelData=array('rowData1'=>$rowData1,'auth_site_id'=>$auth_site_id);
                $response = $this->dispatch(new ValidateExcelGalgotiasJob($excelData));
      ...
      public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
       ...
       $previewPdf = array($request['previewPdf'],$request['previewWithoutBg']);
        ...
        $pdfData=array('studentDataOrg'=>$rowData1,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token);
        //For custom loader
                $link = $this->dispatch(new PdfGenerateGalgotiasJob($pdfData));
    ...
    public function databaseGenerate(){
       ...
       $fetch_degree_data_prog_spec = DB::select( DB::raw('SELECT DISTINCT CONCAT_WS(" ",TRIM(Programme_Name_E),TRIM(Specialization_E)) as prog_spec FROM gu_dc_2 WHERE status="0" GROUP BY prog_spec ORDER BY COUNT(guid) limit 1'));


        foreach ($fetch_degree_data_prog_spec as $value) {
           $progSpec =$value->prog_spec;
            
            $fetch_degree_data = (array)DB::select( DB::raw('SELECT * FROM gu_dc_2 WHERE CONCAT_WS(" ",TRIM(Programme_Name_E),TRIM(Specialization_E)) ="'.$value->prog_spec.'" AND status="0" Limit 1'));          
        ...
        foreach ($fetch_degree_data as $key => $value) {
         ...
         $pdf = new TCPDF('P', 'mm', array('210', '297'), true, 'UTF-8', false);
            ...
            $pdf->SetCreator('SeQR Docs');
            $pdf->SetAuthor('GU SeQR Docs');
            $pdf->SetTitle('GU Certificate');
            $pdf->SetSubject('');
        ...
        for($excel_row = 0; $excel_row < count($fetch_degree_array); $excel_row++)
        {
            ...
            $certName = str_replace("/", "_","2020 ".$fetch_degree_array[0][11].' '.$fetch_degree_array[0][13]).'.pdf';
    ...
    public function pdfGenerate(){
        ...
        $profile_name_hindistr = "eks0 'kgkc [kku";
        $pdf->SetFont($krutidev101, '', $profile_name_hindi_font_size, '', false);
        $pdf->SetTextColor(0,92,88,7,false,'');
        $pdf->SetXY(10, $profile_name_hidiy);
        $pdf->Cell(190, 0, $profile_name_hindistr, 0, false, 'C');
    ...
    public function dbUploadfile(){
        ...
        //Test by Bhavin
        $fetch_degree_data_prog_spec = DB::select( DB::raw('SELECT DISTINCT CONCAT_WS(" ",TRIM(Programme_Name_E),TRIM(Specialization_E)) AS prog_spec, COUNT(guid) FROM gu_dc_3 WHERE STATUS="0" GROUP BY prog_spec ORDER BY COUNT(guid) ASC limit 0,10') );
        ...
        {   
            ...
            $pdfBig->SetTextColor(0, 0, 0, 100, false, '');
                ...
                if($fetch_degree_array[$excel_row][10] == 'DIP'){
                $str = 'rhu o"khZ; fMIyksek ikBîØe '; 
                //Bhavin 22-10-20 Replaced 3 by 2 in hindi 
            ...
            //valid code
             DB::update( DB::raw('UPDATE gu_dc_3 SET status="1" WHERE GUID ="'.$fetch_degree_array[$excel_row][0].'"'));
            }else{

            //valid code
             DB::update( DB::raw('UPDATE gu_dc_3 SET status="2" WHERE GUID ="'.$fetch_degree_array[$excel_row][0].'"'));
    ...
    public function uploadCheckImagePage(){

      return view('admin.galgotias.checkimage');
    }

  public function checkImageExist(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        $template_id=100;
        $get_file_aws_local_flag = $checkUploadedFileOnAwsOrLocal->checkUploadedFileOnAwsORLocal();
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
              ...
              //  print_r($rowData1);
                unset($rowData1[0]);
                $baseUrl=public_path().'/'.$subdomain[0].'/backend/DC_Photo/';
                 $protocol = isset($_SERVER["HTTPS"]) ? 'https' : 'http';
                $path = $protocol.'://'.$subdomain[0].'.'.$subdomain[1].'.com/';
                $imageBaseUrl=$path.$subdomain[0]."/backend/DC_Photo/";
                $strTd='';
                $strTdExist='';
                $strTdExistErr='';
                foreach ($rowData1 as $value) {
                 ...
                 if(isset($value[1])&&!empty($value[1])){
                    $guidImage=$baseUrl.$value[0];
                    $enrollmentNoImage=$baseUrl.$value[1];  
                 }else{
                    $enrollmentNoImage=$baseUrl.$value[0];
                 }   
                ...
                $profile_pic = '';
                $extension = '';
                if(file_exists($enrollmentNoImage.'.jpg.jpg')){
                    $extension = '.jpg.jpg';
                  
                }else if(file_exists($enrollmentNoImage.'.jpeg.jpg')){
                    $extension = '.jpeg.jpg';

                }else if(file_exists($enrollmentNoImage.'.png.jpg')){
                    $extension = '.png.jpg';
                }else if(file_exists($enrollmentNoImage.'.jpg')){
                    $extension = '.jpg';
                }
                else if(file_exists($enrollmentNoImage.'.jpeg')){
                    $extension = '.jpeg';
                }else if(file_exists($enrollmentNoImage.'.png')){
                    $extension = '.png';
                }
        
                if(!empty($extension)){
                   $profile_pic = $value[1].$extension;
                }else if(isset($value[1])&&!empty($value[1])){
                     if(file_exists($guidImage.'.jpg.jpg')){
                        $extension = '.jpg.jpg';
                    }else if(file_exists($guidImage.'.jpeg.jpg')){
                        $extension = '.jpeg.jpg';
                    }else if(file_exists($guidImage.'.png.jpg')){
                        $extension = '.png.jpg';
                    }else if(file_exists($guidImage.'.jpg')){
                        $extension = '.jpg';
                    }
                    else if(file_exists($guidImage.'.jpeg')){
                        $extension = '.jpeg';
                    }else if(file_exists($guidImage.'.png')){
                        $extension = '.png';
                    }
                    if(!empty($extension)){
                    $profile_pic = $value[0].$extension;
                    }
                }

                if(empty($profile_pic)){
                    if(isset($value[1])&&!empty($value[1])){
                    $strTd .='<tr><td>'.$value[0].'</td><td>'.$value[1].'</td></tr>';
                    }else{
                    
                    $strTd .='<tr><td>'.$value[0].'</td></tr>';
                    }
                }else{

                  //  $file_headers = @get_headers($imageBaseUrl.$profile_pic);
                    /*if($value[0]=='18021020009'){

                     $size = getimagesize($imageBaseUrl.$profile_pic);
                     print_r($size);
                     if(!$size){
                        echo 'asd';
                     }
                ...
                $size = getimagesize($imageBaseUrl.$profile_pic);
                    if(!$size) {
                       if(isset($value[1])&&!empty($value[1])){
                        $strTdExistErr .='<tr><td>'.$value[0].'</td><td>'.$value[1].'</td><td><a href="'.$imageBaseUrl.$profile_pic.'" target="_blank"><img src="'.$imageBaseUrl.$profile_pic.'" style="height:50px;width:50px;"></a></td></tr>';
                        }else{
                        
                        $strTdExistErr .='<tr><td>'.$value[0].'</td><td><a href="'.$imageBaseUrl.$profile_pic.'" target="_blank"><img src="'.$imageBaseUrl.$profile_pic.'" style="height:50px;width:50px;"></a></td></tr>';
                        }
                    }
                    else {
                        if(isset($value[1])&&!empty($value[1])){
                        $strTdExist .='<tr><td>'.$value[0].'</td><td>'.$value[1].'</td><td><a href="'.$imageBaseUrl.$profile_pic.'" target="_blank"><img src="'.$imageBaseUrl.$profile_pic.'" style="height:50px;width:50px;"></a></td></tr>';
                        }else{
                        
                        $strTdExist .='<tr><td>'.$value[0].'</td><td><a href="'.$imageBaseUrl.$profile_pic.'" target="_blank"><img src="'.$imageBaseUrl.$profile_pic.'" style="height:50px;width:50px;"></a></td></tr>';
                        }
                    }
            ...
            if($imagesType==0){
                if(!empty($strTdExist)){
                     return response()->json(['success'=>true, 'type'=>1,'message' => 'Existed images!','strTd'=>$strTd,'strTdExist'=>$strTdExist,'strTdExistErr'=>$strTdExistErr,'imagesType'=>$imagesType]);
                }else{
                     return response()->json(['success'=>false,'type'=>2, 'message' => 'Images not found!','strTd'=>$strTd,'strTdExist'=>$strTdExist,'strTdExistErr'=>$strTdExistErr,'imagesType'=>$imagesType]);
                }
               
            }else{
            if(!empty($strTd)){
                
                 return response()->json(['success'=>false,'type'=>2, 'message' => 'Some images not found!','strTd'=>$strTd,'strTdExist'=>$strTdExist,'strTdExistErr'=>$strTdExistErr,'imagesType'=>$imagesType]);
                
            }else{
                 return response()->json(['success'=>true, 'type'=>1,'message' => 'All images exists!','strTd'=>$strTd,'strTdExist'=>$strTdExist,'strTdExistErr'=>$strTdExistErr,'imagesType'=>$imagesType]);
            }
            }

            }else{
            return response()->json(['success'=>false,'type'=>0,'message'=>'File not uploaded!']);
        }

           
         }
        else{
            return response()->json(['success'=>false,'type'=>0,'message'=>'Please upload valid file!']);
        }

        }
        else{
            return response()->json(['success'=>false,'type'=>0,'message'=>'File not found!']);
        }
