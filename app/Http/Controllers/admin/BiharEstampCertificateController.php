...
use App\Jobs\PdfGenerateBiharEstampCertificateUtilityJob;
use App\Jobs\PdfGenerateBiharEstampEcourtUtilityJob;
use App\Jobs\PdfGenerateBiharEcourtFeeUtilityJob;
use App\Jobs\ValidateExcelBiharEstampCertificateUtilityJob;
...
class BiharEstampCertificateController extends Controller
{
    ...
    public function uploadpageApiUtility(){
        return view('admin.biharEstamp.apiutility'); 
    } 

   public function uploadpagePrinterUtility(){
        return view('admin.biharEstamp.printerutility'); 
    }

    public function uploadpageutility(){
        return view('admin.biharEstamp.utility'); 
    }
    ...
    public function validateExcelUtility(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            $destPath =public_path().'/'.$subdomain[0].'/backend/sample_excel/'.$fileName;
                ...
                $response = $this->dispatch(new ValidateExcelBiharEstampCertificateUtilityJob($excelData));
                //print_r($response);
                $responseData =$response->getData();
    ...
    public function uploadfileUtility(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        switch($dropdown_template_id){
                case '1' : 
                                $link = $this->dispatch(new PdfGenerateBiharEstampCertificateUtilityJob($pdfData));
                             break;
                case '2' : 
                                $link = $this->dispatch(new PdfGenerateBiharEstampEcourtUtilityJob($pdfData));
                             break;
                case '3' : 
                                $link = $this->dispatch(new PdfGenerateBiharEcourtFeeUtilityJob($pdfData));
                             break;
                case '4' : 
                                $link = $this->dispatch(new PdfGenerateBiharEcourtFeeUtilityJob($pdfData));
                             break;
        ...
        if(!empty($_POST['printer_name_network'])){
                preg_match_all('/<a[^>]+href=([\'"])(?<href>.+?)\1[^>]*>/i', $link, $result);
