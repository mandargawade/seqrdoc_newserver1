...
class DegreeCertificateController extends Controller
{
    ...
    public function uploadfile(Request $request){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $aws_excel = \File::copy($fullpath,public_path().'\127\backend\templates\/'.$template_data['id'].'\/'.$excelfile);
        ...
        if(isset($FID['bg_template_id']) && $FID['bg_template_id'] != '') {
            if($FID['bg_template_id'] == 0) {
                ...
                else
                {
                    ...
                    $bg_template_img = public_path().'\127\backend\canvas\bg_images\degree_certificate_bg.jpg';
        ...
        if(isset($FID['bg_template_id']) && $FID['bg_template_id'] != '') {
            if($FID['bg_template_id'] == 0) {
                ...
                else
                {
                    ...
                    $bg_template_img_generate = 'C:\xampp\htdocs\seqr\public\127\backend\canvas\bg_images\degree_certificate_bg.jpg';
    ...
    public function databaseGenerate(){
       ...
       $fetch_degree_data_prog_spec = DB::select( DB::raw('SELECT DISTINCT CONCAT_WS(" ",TRIM(Programme_Name_E),TRIM(Specialization_E)) as prog_spec FROM gu_dc WHERE Programme_Name_E ="BACHELOR OF SCIENCE"') );
        ...
        for($excel_row = 0; $excel_row < count($fetch_degree_array); $excel_row++)
        {
            ...
            $serial_no = trim($fetch_degree_array[$excel_row][0]);
                ...
                $repeatx= 0;
                    ...
                    $name_repeatstr = strtoupper(trim($fetch_degree_array[$excel_row][4])).' '.'CGPA '.$cgpaFormat.' '.strtoupper(trim($fetch_degree_array[$excel_row][8])).' '; 
                ...
                $name = str_replace(' ','',substr(trim(strtoupper($fetch_degree_array[$excel_row][4])), 0, 6));
         ...
         $certName = str_replace("/", "_", $progSpec) .".pdf";
    ...
    public function pdfGenerate(){
        ...
        //set fonts
        $arial = TCPDF_FONTS::addTTFfont(public_path().'\127\backend\canvas\fonts\Arial.TTF', 'TrueTypeUnicode', '', 96);
        $arialb = TCPDF_FONTS::addTTFfont(public_path().'\127\backend\canvas\fonts\Arial Bold.TTF', 'TrueTypeUnicode', '', 96);

        $krutidev100 = TCPDF_FONTS::addTTFfont(public_path().'\127\backend\canvas\fonts\K100.TTF', 'TrueTypeUnicode', '', 96); 
        $krutidev101 = TCPDF_FONTS::addTTFfont(public_path().'\127\backend\canvas\fonts\K101.TTF', 'TrueTypeUnicode', '', 96);
        $HindiDegreeBold = TCPDF_FONTS::addTTFfont(public_path().'\127\backend\canvas\fonts\KRUTI_DEV_100__BOLD.TTF', 'TrueTypeUnicode', '', 96); 
        $arialNarrowB = TCPDF_FONTS::addTTFfont(public_path().'\127\backend\canvas\fonts\ARIALNB.TTF', 'TrueTypeUnicode', '', 96);
        $timesNewRomanBI = TCPDF_FONTS::addTTFfont(public_path().'\127\backend\canvas\fonts\Times-New-Roman-Bold-Italic.TTF', 'TrueTypeUnicode', '', 96);
        $timesNewRomanI = TCPDF_FONTS::addTTFfont(public_path().'\127\backend\canvas\fonts\Times New Roman_I.TTF', 'TrueTypeUnicode', '', 96);
        ...
        $template_img_generate = public_path().'\127\backend\canvas\bg_images\INT_lite background.jpg';
        ...
        $qr_code_path = public_path().'\127\backend\canvas\images\qr\/'.$codeContents.'.png';
        $qrCodex = 5.3;
        $qrCodey = 17.9;
        $qrCodeWidth =26.3;
        $qrCodeHeight = 25.3;
        ...
        //profile photo
        $profile_path = public_path().'\127\backend\templates\444\degree_certi.jpg';
        $profilex = 181;
        $profiley = 19.8;
        $profileWidth = 22.2;
        $profileHeight = 26.6;
        ...
        $textArray = imagettfbbox(1.4, 0, public_path().'\127\backend\canvas\fonts\Arial Bold.TTF', $microlinestr);
        $strWidth = ($textArray[2] - $textArray[0]);
        $strHeight = $textArray[6] - $textArray[1] / 1.4;
        ...
        $textArrayEnrollment = imagettfbbox(1.4, 0, public_path().'\127\backend\canvas\fonts\Arial Bold.TTF', $microlineEnrollment);
        $strWidthEnrollment = ($textArrayEnrollment[2] - $textArrayEnrollment[0]);
        $strHeightEnrollment = $textArrayEnrollment[6] - $textArrayEnrollment[1] / 1.4;
        ...
        //uv sign invisible
        $uv_sign_invisible_path = public_path().'\127\backend\templates\444\uv_visible_sign.png';
        ...
        //uv sign visible
        $uv_sign_visiblle_path = public_path().'\127\backend\templates\444\uv_invisible_sign.png';
        ...
        //vc sign visible
        $uv_sign_visiblle_path = public_path().'\127\backend\templates\444\VC_Sign.png';
        ...
        //ce sign visible
        $uv_sign_visiblle_path = public_path().'\127\backend\templates\444\CE_Sign.png';
    ...
    public function dbUploadfile(){
        ...
        for($excel_row = 0; $excel_row < count($fetch_degree_array); $excel_row++)
        {
            ...
            $pdfBig->SetTextColor(0, 0, 0, 100, false, '');
                ...
                //name // enrollment // degree // branch // cgpa // guid
                if($fetch_degree_array[$excel_row][10] == 'DB' || $fetch_degree_array[$excel_row][10] == 'INT' || $fetch_degree_array[$excel_row][10] == 'DIP'){
                    $codeContents = trim($fetch_degree_array[$excel_row][4])."\n".trim($fetch_degree_array[$excel_row][1])."\n".trim($fetch_degree_array[$excel_row][11])."\n".trim($fetch_degree_array[$excel_row][13])."\n".number_format(trim($fetch_degree_array[$excel_row][6]),2)."\n\n".md5(trim($fetch_degree_array[$excel_row][0]));
                }
                else if($fetch_degree_array[$excel_row][10] == 'DO'){
                    $codeContents = trim($fetch_degree_array[$excel_row][4])."\n".trim($fetch_degree_array[$excel_row][1])."\n".trim($fetch_degree_array[$excel_row][11])."\n".number_format(trim($fetch_degree_array[$excel_row][6]),2)."\n\n".md5(trim($fetch_degree_array[$excel_row][0]));
                }
                ...
                $invisible2str = 'CGPA '.number_format(trim($fetch_degree_array[$excel_row][6]),2); 
                ...
                $repeatx= 0;
                    ...
                    //name repeat line
                    $name_repeaty = 242.9;
                    $name_repeatstr = strtoupper(trim($fetch_degree_array[$excel_row][4])).' '.'CGPA '.number_format(trim($fetch_degree_array[$excel_row][6]),2).' '.strtoupper(trim($fetch_degree_array[$excel_row][8])).' '; 
                    $name_repeatstr .= $name_repeatstr . $name_repeatstr . $name_repeatstr . $name_repeatstr . $name_repeatstr;
