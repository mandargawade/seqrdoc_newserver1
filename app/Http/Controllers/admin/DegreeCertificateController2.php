...
class DegreeCertificateController extends Controller
{
    ...
    public function databaseGenerate(){
       ...
       $fetch_degree_data_prog_spec = DB::select( DB::raw('SELECT DISTINCT CONCAT_WS(" ",TRIM(Programme_Name_E),TRIM(Specialization_E)) as prog_spec FROM gu_dc_2 WHERE Type ="DIP" LIMIT 1') );

        foreach ($fetch_degree_data_prog_spec as $value) {
           $progSpec =$value->prog_spec;

             $fetch_degree_data = (array)DB::select( DB::raw('SELECT * FROM gu_dc_2 WHERE CONCAT_WS(" ",TRIM(Programme_Name_E),TRIM(Specialization_E)) ="'.$value->prog_spec.'" LIMIT 2'));
        ...
        for($excel_row = 0; $excel_row < count($fetch_degree_array); $excel_row++)
        {
            ...
            if($fetch_degree_array[$excel_row][10] == 'DB'){
                ...
                $template_img_generate = public_path().'/'.$subdomain[0].'/backend/canvas/bg_images/GU Nursing background_light.jpg';   
            }
            ...
            $serial_no = trim($fetch_degree_array[$excel_row][0]);
                ...
                if($fetch_degree_array[$excel_row][10] == 'NU'){
                    ...
                    if(strpos($fetch_degree_array[$excel_row][17], 'With Distinciton') !== false){
    ...
    public function dbUploadfile(){
        ...
        foreach ($fetch_degree_data_prog_spec as $value) {
           $progSpec =$value->prog_spec;
             ...
             $fetch_degree_data = (array)DB::select( DB::raw('SELECT * FROM gu_dc_2 WHERE CONCAT_WS(" ",TRIM(Programme_Name_E),TRIM(Specialization_E)) ="'.$value->prog_spec.'" LIMIT 10'));
            $fetch_degree_data = collect($fetch_degree_data)->map(function($x){ return (array) $x; })->toArray();
        ...
        $file_name = $fetch_degree_array[0][11].'_'.$fetch_degree_array[0][13].'_'.$fetch_degree_array[0][10].'.pdf';
