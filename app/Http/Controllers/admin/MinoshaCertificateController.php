...
use Helper;
use App\Jobs\ValidateExcelMinoshaJob;
use App\Jobs\PdfGenerateMinoshaJob;
use App\Jobs\PdfGenerateMinoshaJob2;

class MinoshaCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.minosha.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelMinoshaJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
		...
		if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateMinoshaJob($pdfData));
        }else if($dropdown_template_id==2){
             $link = $this->dispatch(new PdfGenerateMinoshaJob2($pdfData));
        }
