...
use Helper;
use App\Jobs\ValidateExcelCedpJob;
use App\Jobs\PdfGenerateCedpJob;
use App\Jobs\PdfGenerateCedpJob2;
use App\Jobs\PdfGenerateCedpJob3;
class CedpCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.cedp.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelCedpJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        $template_id = 100;
        $dropdown_template_id = $request['template_id'];
        /* 1=Marksheet 2=Certificate 3=DMLT */
       ...
       // $tmpDir = $this->createTemp(public_path().'\backend\images\ghosttemp\temp');
        ...
        $pdfData=array('studentDataOrg'=>$rowData1,'subjectsMark'=>$rowData2, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token); //For Custom Loader
        
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateCedpJob($pdfData));
        }
        elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateCedpJob2($pdfData));
        }
		elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateCedpJob3($pdfData));
        }
