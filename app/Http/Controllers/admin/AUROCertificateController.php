...
use Helper;
use App\Jobs\ValidateExcelAUROJob;
use App\Jobs\PdfGenerateAUROJob;
use App\Jobs\PdfGenerateAUROJob_2022;
use App\Jobs\PdfGenerateAUROJob_PHD; 
class AUROCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.auro.index');
    }
    ...
    public function pdfGenerate(){
        ...
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, "EN ROLL NO: 012345678", 0, false, 'L'); 
        ...
        $pdf->Image($invisible_logo, $x, $y, '55', '29', "PNG", '', 'R', true);
        ...
        $x = 14;
        $y = 47; 
        $invisible_namestr = 'Student Name';
        $pdf->SetFont($verdana, 'B', 8.5, '', false);
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, $invisible_namestr, 0, false, 'L');

        $x = 14;
        $y = 52; 
        $invisible_namestr = 'Bachelor of Science in Hospitality Mangamement';
        $pdf->SetFont($verdana, '', 8.5, '', false);
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, $invisible_namestr, 0, false, 'L');
        ...
        $x = 14.2;
        $y = 56; 
        $invisible_namestr = 'Culinary and Patisserie';
        $pdf->SetFont($verdana, 'B', 8.5, '', false);
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, $invisible_namestr, 0, false, 'L');
        ...
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, 'School of Hospitality', 0, false, 'C');
        ...
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, 'Student Name', 0, false, 'C');

        //$x = 117;
        $y = 118; 
        $pdf->SetFont($danishRegular, '', 27, '', false);
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, 'having satisfied all the degree requiremnets', 0, false, 'C');

        //$x = 148;
        $y = 129; 
        $pdf->SetFont($danishRegular, '', 27, '', false);
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, 'has been conferred', 0, false, 'C');

        //$x = 60;
        $y = 145.5; 
        $pdf->SetFont($oldEnglishFive, '', 25, '', false);
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, 'Bachelor of Science in Hospitality Mangamement', 0, false, 'C');

        //$x = 110;
        $y = 163; 
        $pdf->SetFont($danishRegular, '', 27, '', false);
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, 'With Specialization in Culinary and Patisserie', 0, false, 'C');
        ...
        $pdf->SetFont($danishRegular, '', 27, '', false);
        $pdf->SetXY($x, $y);
        $pdf->Cell(0, 0, 'signed and sealed at Surat, Gujarat, this twelth day of january in the year two thousand and twenty two', 0, false, 'C');
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                //dd($excelData);
                $response = $this->dispatch(new ValidateExcelAUROJob($excelData));
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateAUROJob($pdfData));
        }
		elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateAUROJob_2022($pdfData));
        }
		elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateAUROJob_PHD($pdfData));
        }
