...
use Helper;
use App\Jobs\ValidateExcelRRMUJob;
use App\Jobs\ValidateExcelRRMUJob1;
use App\Jobs\PdfGenerateRRMUJob;
use App\Jobs\PdfGenerateRRMUJob1;
class RRMUCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.rrmu.index');
    }
    ...
    /*public function pdfGenerate(){
        ...
        $fontEBPath=public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\E-13B_0.php';
            ...
            $tmpDir = $this->createTemp(public_path().'\backend\images\ghosttemp\temp_rrmu');
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                if($dropdown_template_id == 1)
                {
                    ...
                    //dd($excelData);
                    $response = $this->dispatch(new ValidateExcelRRMUJob($excelData));
                    ...
                    //dd($excelData);
                    $response = $this->dispatch(new ValidateExcelRRMUJob1($excelData));
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateRRMUJob($pdfData));
        }else if($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateRRMUJob1($pdfData));
        }
    ...
    /*public function CreateMessage($tmpDir, $name = "",$font_size,$print_color){ // for debugging
        $AlphaPosArray = array(
            "A" => array(0, 849),
            "B" => array(849, 864),
            "C" => array(1713, 840),
