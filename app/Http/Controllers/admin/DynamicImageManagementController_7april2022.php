    ...
    public function displayImage($sortBy,$value,$searchkey='',CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        foreach ($filenameArray as $key => $value) {
	    	...
	    	if($get_file_aws_local_flag->file_aws_local == '1'){
	        	...
	        	$filenameArray=glob(public_path().'/'.$subdomain[0].'/backend/templates/'.$folder_name."/*");
	        } 
		...
		if($sortBy == 'atoz'){
            ...
            if (in_array($file_type, $file_display) == true) {
		   	...
		   	return response()->json(['data'=>$imagesArray,'get_file_aws_local_flag'=>$get_file_aws_local_flag,'systemConfig'=>$systemConfig['sandboxing'],'template_name'=>$template_name]);
		}else{
