...
class DegreeCertificateController extends Controller
{
    ...
    public function databaseGenerate(){
       ...
       $fetch_degree_data_prog_spec = DB::select( DB::raw('SELECT DISTINCT CONCAT_WS(" ",TRIM(Programme_Name_E),TRIM(Specialization_E)) as prog_spec FROM gu_dc_3 WHERE status="0" GROUP BY prog_spec ORDER BY COUNT(guid) limit 40,10'));
