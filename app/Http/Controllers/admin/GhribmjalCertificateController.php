...
use Helper;
use App\Jobs\ValidateExcelGhribmjalJob;
use App\Jobs\PdfGenerateGhribmjalBBAFirstYearJob;
use App\Jobs\PdfGenerateGhribmjalMBAFirstYearJob;
use App\Jobs\PdfGenerateGhribmjalMBAJob;


class GhribmjalCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.Ghribmjal.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelGhribmjalJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
		...
		if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateGhribmjalBBAFirstYearJob($pdfData));
        }
        elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateGhribmjalMBAFirstYearJob($pdfData));
        }
        elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateGhribmjalMBAJob($pdfData));
        }
