...
use Helper;
use App\Jobs\ValidateExcelSangamuniJob;
use App\Jobs\PdfGenerateSangamuniJob;
use App\Jobs\PdfGenerateSangamuniPharmacyJob;
use App\Jobs\PdfGenerateSangamuniPhdJob;
use App\Jobs\PdfGenerateSangamuniDHAJob;
use App\Jobs\PdfGenerateSangamuniAgJob;
use App\Jobs\PdfGenerateSangamuniDiplomaJob;

class SangamuniCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.sangamuni.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelSangamuniJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
		...
		if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateSangamuniJob($pdfData));
        }
        elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateSangamuniPharmacyJob($pdfData));
        }
        elseif($dropdown_template_id==3 || $dropdown_template_id==6){
            $link = $this->dispatch(new PdfGenerateSangamuniPhdJob($pdfData));
        }
        elseif($dropdown_template_id==4){
            $link = $this->dispatch(new PdfGenerateSangamuniDHAJob($pdfData));
        }
        elseif($dropdown_template_id==5){
            $link = $this->dispatch(new PdfGenerateSangamuniAgJob($pdfData));
        }
        elseif($dropdown_template_id==7){
            $link = $this->dispatch(new PdfGenerateSangamuniDiplomaJob($pdfData));
        }
