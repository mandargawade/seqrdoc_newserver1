...
use App\Jobs\PdfGenerateMolwaReport;
...
{        
        ...
        if($api_option==2){  
            ...
            if (is_array($object2['records']) || is_object($object2['records'])){
                ...
                {  
                    ...
                    $input['created_at'] = now();   // date("Y-m-d H:i:s")
    ...
    public function ExportToExcel(Request $request, $id, $flag='')
    {
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            $value = $spreadsheet->getSheet(0)->getCell('J'.$row)->getValue();
            $spreadsheet->getSheet(0)->getCell('J'.$row)->setValueExplicit($value, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
        }		
        $sheet_dimension = $spreadsheet->getSheet(0)->calculateWorksheetDimension(); // Get sheet dimension
        // Apply text format to numbers
        $spreadsheet->getSheet(0)->getStyle($sheet_dimension)->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_TEXT);
    ...
    public function IsNullString($ff_data) {
        if(is_null($ff_data)==1){
            return '';
        }else{ 
            return $ff_data;
			/*if (strlen($ff_data) != strlen(utf8_decode($ff_data))) {
                return $ff_data; //return false; Data is not in english.
            } else {
                return ''; //return true; Data is in english.
            }*/
        }
    }

	public function CompressPDF($pdffile, $sub_domain, $pdfSetting) {
		$inputFile=\Config::get('constant.directoryPathBackward')."\\".$sub_domain."\\backend\\pdf_file\\".$pdffile;
		$outputFile=\Config::get('constant.directoryPathBackward')."\\".$sub_domain."\\backend\\pdf_file\\1_".$pdffile;
		/*$inputFile="C:\\wamp64\\www\\demo\\public\\".$sub_domain."\\backend\\pdf_file\\".$pdffile;
		$outputFile="C:\\wamp64\\www\\demo\\public\\".$sub_domain."\\backend\\pdf_file\\1_".$pdffile;*/
		CoreHelper::compressPdfFile($inputFile, $outputFile, $pdfSetting);
		unlink($inputFile);
    ...
    public function IdGenerate(Request $request){
        ...
        foreach ($data as $sheet_one) { 
            ...
            $pdfBig->Image($template_img_generate, 0, 0, 86, 54, 'jpg', '', true, false);
            $pdf->Image($template_img_generate, 0, 0, 86, 54, 'jpg', '', true, false);
            
            $fid=$sheet_one->id;
            $ff_id=$sheet_one->ff_id;
            $ff_name=$this->IsNullString($sheet_one->ff_name);
            $fh_name=$this->IsNullString($sheet_one->father_or_husband_name);
            $mother_name=$this->IsNullString($sheet_one->mother_name);
            $post_office=$this->IsNullString($sheet_one->post_office);
            $post_code=$sheet_one->post_code;
            $district=$this->IsNullString($sheet_one->district);
            $ut_name=$this->IsNullString($sheet_one->upazila_or_thana);
            $vw_name=$this->IsNullString($sheet_one->village_or_ward);
            $nid=$sheet_one->nid;
            $ghost_image_code=$sheet_one->ghost_image_code;
            $ff_photo=$sheet_one->ff_photo; 
            $serial_no='ID_'.$ff_id;
            ...
            if($ff_photo != ''){
                ...
                if ($pic !== false){
                    ...
                    if (file_put_contents($TEMPIMGLOC, $decodedImg) !== false) {
						...
						$pdfBig->Image($TEMPIMGLOC, 5.1, 16.9, 18.6, 19.2, $pic[1], '', true, false);
                        $pdf->Image($TEMPIMGLOC, 5.1, 16.9, 18.6, 19.2, $pic[1], '', true, false);
                    }
                    unlink($TEMPIMGLOC); 
                }                   
            }             
			...
			$pdfBig->AutosizeText($ff_name, 46, 'nikosh', 'B', 9 );
			...
			$pdfBig->AutosizeText($ut_name, 25, 'nikosh', '', 9 );
            //Name of District
            ...
            $pdf->SetXY(38.7,20.9);        
            //$pdf->MultiCell(48.5, 0, $ff_name, 0, 'L');
			$pdf->AutosizeText($ff_name, 46, 'nikosh', 'B', 9 );	
            $pdf->SetFont('nikosh', '', 9);
            //Father/Husband Name of FF
            $pdf->SetXY(35.6,24.8);        
            $pdf->MultiCell(50, 0, $fh_name, 0, 'L');    
            ...
            $pdf->SetXY(39,32.1);
            //$pdf->MultiCell(46.5, 0, $ut_name, 0, 'L');
			$pdf->AutosizeText($ut_name, 25, 'nikosh', '', 9 );
            //Name of District
            $pdf->SetXY(31,35.7);
            $pdf->MultiCell(55, 0, $district, 0, 'L');         
			...
			$pdffile=$serial_no.'.pdf';
			$sub_domain=$subdomain[0];
			$this->CompressPDF($pdffile, $sub_domain, 'ebook');
    ...
    public function CertificateGenerate(Request $request){
        ...
        foreach ($data as $sheet_one) { 
            ...
            */
            ...
            //Father/Husband Name of FF
            $pdfBig->SetXY(64,100);
            $pdfBig->MultiCell(200, 7, $fh_name, 0, 'L');  
            //Name of Village/Ward
            $pdfBig->SetXY(64,111);
            $pdfBig->MultiCell(0, 7, $vw_name, 0, 'L');
            //Name of Post Office
            $pdfBig->SetXY(64,121);
            ...
            //Name of District
            $pdfBig->SetXY(64,131);
            $pdfBig->MultiCell(0, 7, $district, 0, 'L');
            ...
            //Father/Husband Name of FF
            $pdf->SetXY(64,100);
            $pdf->MultiCell(200, 7, $fh_name, 0, 'L');  
            //Name of Village/Ward
            $pdf->SetXY(64,111);
            $pdf->MultiCell(0, 7, $vw_name, 0, 'L');
            //Name of Post Office
            $pdf->SetXY(64,121);
            $pdf->MultiCell(90, 7, $post_office, 0, 'L');  
            //Upazila/Thana Name
            ...
            $ghostImagey = 197.5;
            $name = substr(str_replace(' ','',strtoupper($nameOrg)), 0, 11);
			$this->createghostimg($ff_id, $tmpDir, $name, $ghost_font_size, $subdomain[0]);
			$pdfBig->Image("$tmpDir/" . $name."".$ghost_font_size.".png", $ghostImagex, $ghostImagey, 80, 7, 'png', '', true, false);
			$pdf->Image("$tmpDir/" . $name."".$ghost_font_size.".png", $ghostImagex, $ghostImagey, 80, 7, 'png', '', true, false);
            ...
            */
			...
			$pdffile=$serial_no.'.pdf';
			$sub_domain=$subdomain[0];
			$this->CompressPDF($pdffile, $sub_domain, 'screen');			
    ...
    public function StatusComplete(Request $request){
        ...
        return response()->json(['success'=>true, 'uid'=>$id, 'col'=>$col]);
    }

    public function getCtCompleteProgess(){
        $filename = Session::get('CcompleteprogressFile');
        $domain = \Request::getHost();        
        $subdomain = explode('.', $domain);     
        if($subdomain[0] != "secura"){$subdomain[0]= "secura";}
        $file = public_path().'\\'.$subdomain[0].'\\progess_files\\'.$filename;              
    ...
    public function sendUpdateToMolwa(Request $request){
        $domain = \Request::getHost();        
        $subdomain = explode('.', $domain);     
        if($subdomain[0] != "secura"){$subdomain[0]= "secura";}
        $id=$request['id']; //record_unique_id
        ...
        if($record_type=='import'){
            $records = ImportLogDetail::where('record_unique_id',$id)->first();
            $import_date=ImportLogDetail::where('record_unique_id',$id)->first()->created_at;
            $import_date=date('Y-m-d h:i:s', strtotime($import_date));
        }else{
            $records = ReImportLogDetail::where('record_unique_id',$id)->first();
            $import_date=ReImportLogDetail::where('record_unique_id',$id)->first()->created_at;
            $import_date=date('Y-m-d h:i:s', strtotime($import_date));
        }
        Session::forget('CcompleteprogressFile');
        Session::save();

        $filename='progress_Ccompleted_'.uniqid().'.txt';
        $file = public_path().'\\'.$subdomain[0].'\\progess_files\\'.$filename;              
        if (!file_exists($file)) {
            $myfile=fopen($file, "w");
            $txt= array('percent'=>0, 'show_msg'=>'');
        ...
        Session::put('CcompleteprogressFile', $filename);
        Session::save();  

        /*Start Smart Card Issue Info Update API*/
        $apiKey="";
        $data_send = FreedomFighterList::where('record_unique_id',$id)->where('active','Yes')->get(); 
        $recordCount = $data_send->count(); 
        
        $cnt=1;
        foreach ($data_send as $value) {
            $ff_id=$value->ff_id;
            $import_status=$value->import_flag;
            ...
            if($value->c_generated_at != ''){
                $c_generated_at=$value->c_generated_at;
            }else{
                $c_generated_at="";
            }
            if($value->c_completed_at != ''){
                $c_completed_at=$value->c_completed_at;
            }else{
                $c_completed_at="";
            }
            if($col == 'idcard_status'){
                ...
                $ffload = array(
                    ...
                    'idcard_generate_status' => "ID CARD".$initial_str."GENERATED",
                    'idcard_generate_date' => $generated_at,
                    'idcard_complete_status' => "ID CARD".$initial_str."COMPLETED",
                    'idcard_complete_date' => $completed_at,
                );
            }else{
                ...
                $ffload = array(
                    ...
                    'import_status' => $import_status,
                    'import_date' => $import_date,
                    'certificate_generate_status' => "CERTIFICATE".$initial_str."GENERATED",
                    'certificate_generate_date' => $c_generated_at,
                    'certificate_complete_status' => "CERTIFICATE".$initial_str."COMPLETED",
                    'certificate_complete_date' => $c_completed_at,
                );
            } 
            $curl = curl_init();
            $opts = [
                    CURLOPT_URL => 'http://mis.molwa.gov.bd/api/smart-card-issue-info-update',
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_CUSTOMREQUEST => 'POST',
                    CURLOPT_POST => 1,
                    CURLOPT_POSTFIELDS => $ffload,
            ];
            curl_setopt($curl, CURLOPT_HTTPHEADER, array('api_key: ' . $apiKey));
            curl_setopt_array($curl, $opts);
            $response = curl_exec($curl);
            $json2 = json_decode(json_encode($response), true);
            $object2 = json_decode( $json2, true );
            print_r($response);
            $statusCode=$object2['statusCode'];
            curl_close($curl);
            if($statusCode!="200"){
                $file_name=date('d-m-Y').'.txt';
                $filepath = public_path().'\\'.$subdomain[0].'/molwa_api_error/'.$file_name; 
                $txt = "Date: ".date('d-m-Y H:i:s')."| Document: ".$document." | FF ID: ".$ff_id." | Record Unique ID: ".$id." | Response: ".$response."\n";
                if(!is_file($filepath)){
        ...
        /*End Smart Card Issue Info Update API*/
    ...
    public function ViewImportFF(Request $request){        
        ...
        <table class='table table-hover table-bordered'>
            ...
            <h5><strong>Name:</strong> $data->name | <strong>Per Page:</strong> $data->per_page | <strong>Page Number:</strong> $data->page_no | <strong>Quantity:</strong> $data->fresh_records | <strong>Repeat:</strong> $data->repeat_records</h5> 
            $params
            </caption>
            <tr>
                <th width='15%'></th>
    ...
    public function ViewFF(Request $request){        
        ...
        if($flag=='Process'){
            $tbl="<h4>Process Log</h4><table class='table table-bordered'>";
            ...
            $tbl .="</table>";
            $tbl.="<h4>Activate/Deactivate Log</h4><table class='table table-bordered'>";
            $tbl .="<tr class='bg-primary'><td class='tdtext'>Status</td><td class='tdtext'>Reason</td><td class='tdtext'>Date</td></tr>";
                    $reason_data=FFReasonLog::where('record_id',$id) -> orderBy('id', 'asc') -> get();
					$rc=1;
					foreach ($reason_data as $rdata){
						$tbl .="<tr><td>".$rdata->status."</td><td>$rdata->reason</td><td>".date('d-m-Y h:i A', strtotime($rdata->created_at))."</td></tr>";							
                    	$rc++;
					}
            $tbl .="</table>";
            echo $tbl;
            ...
            if($flag == 'history'){
				...
				<td width='50%' style='vertical-align: top !important;'>
					<table class='table table-bordered'>
						<tr><th width='30%'>Freedom Fighter ID</th><td width='80%'>$ff_id</td></tr>
						<tr><th>Freedom Fighter Name</th><td>$ff_name</td></tr>
						<tr><th>Father/Husband Name</th><td>$fh_name</td></tr>
						<tr><th>Mother Name</th><td>$mother_name</td></tr>
						<tr><th>Alive</th><td>$is_alive</td></tr>
				...
				<td width='40%' style='vertical-align: top !important;border:1px solid #ddd;'>
					<form method='post' id='processEditForm' novalidate='novalidate' autocomplete='off'>
					<div class='row'>
						<div class='form-group' style='width: 100% !important;'>						
							<div class='col-md-12'>
								<label class='control-label'>Freedom Fighter Name</label>
								<input type='text' class='form-control' style='width: 100% !important;' id='ff_name' name='ff_name' value='".$ff_name."'>
							</div>
							<div class='col-md-12' style='margin-top:1rem!important;'>
								<label class='control-label'>Father/Husband Name</label>
								<input type='text' class='form-control' style='width: 100% !important;' id='fh_name' name='fh_name' value='".$fh_name."'>
							</div>
							<div class='col-md-12' style='margin-top:1rem!important;'>
								<label class='control-label'>Post Office</label>
								<input type='text' class='form-control' style='width: 100% !important;' id='post_office' name='post_office' value='".$post_office."'>
							</div>
							<div class='col-md-12' style='margin-top:1rem!important;'>
								<label class='control-label'>District</label>
								<input type='text' class='form-control' style='width: 100% !important;' id='district' name='district' value='".$district."'>
							</div>
							<div class='col-md-12' style='margin-top:1rem!important;'>
								<label class='control-label'>Upazila/Thana</label>
								<input type='text' class='form-control' style='width: 100% !important;' id='ut_name' name='ut_name' value='".$ut_name."'>
							</div>
							<div class='col-md-12' style='margin-top:1rem!important;'>
								<label class='control-label'>Village/Ward</label>
								<input type='text' class='form-control' style='width: 100% !important;' id='vw_name' name='vw_name' value='".$vw_name."'>
							</div> 

						</div>			
					</div>
					<div class='row'>
						<div class='col-md-12 text-center' style='margin-top:0.5rem!important;margin-bottom:0.5rem!important;'><small>View preview before editing the record.</small></div>
						<div class='col-md-6'>
							<button type='button' class='btn btn-info' id='previewC' rid='".$id."' flag='Preview'>Preview Certificate</button>
							<br /><span id='pc_msg'></span>
						</div>
						<div class='col-md-6 text-right'>
							<button type='button' class='btn btn-info' id='previewID' rid='".$id."' flag='Preview'>Preview ID</button>
							<br /><span id='id_msg'></span>
						</div>
					</div>
					<div class='row'>
						<div class='col-md-12 text-center' style='margin-top:1rem!important;'>
							<button type='submit' class='btn btn-primary editData' id='editData'>Edit & Save Changes</button>
							<input type='hidden' class='form-control' style='width: 100% !important;' id='record_id' name='record_id' value='".$id."'>
						</div>
						<div class='col-md-12 text-center'><div id='show_msg' class='text-success'></div></div>					
					</div>					
					<div class='row'>
						<div class='col-md-12 text-center' style='margin-top:0.5rem!important;margin-bottom:0.5rem!important;'><small>Generate live PDF after editing the record.</small></div>
						<div class='col-md-6'>
							<button type='button' class='btn btn-success' id='liveC' rid='".$id."' flag='Live' disabled>Live Certificate</button>
							<br /><span id='lpc_msg'></span>
						</div>
						<div class='col-md-6 text-right'>
							<button type='button' class='btn btn-success' id='liveID' rid='".$id."' flag='Live' disabled>Live ID</button>
							<br /><span id='lid_msg'></span>
						</div>
					</div>
					</form>
				</td>
    ...
    public function ActiveInactiveRecord(Request $request){        
        ...
        if($id != ''){            
            ...
            if($ffid != ''){
                if($mode=='active'){ 
                    DB::select(DB::raw('UPDATE `student_table` SET status=1 WHERE serial_no="ID_'.$ffid.'" ORDER BY id DESC LIMIT 1'));
                    DB::select(DB::raw('UPDATE `student_table` SET status=1 WHERE serial_no="C_'.$ffid.'" ORDER BY id DESC LIMIT 1'));
                }else{
                    DB::select(DB::raw('UPDATE `student_table` SET status=0 WHERE serial_no="ID_'.$ffid.'" ORDER BY id DESC LIMIT 1'));
                    DB::select(DB::raw('UPDATE `student_table` SET status=0 WHERE serial_no="C_'.$ffid.'" ORDER BY id DESC LIMIT 1'));
                }
            }
            $reason = new FFReasonLog(); 
			$reason->record_id = $id; 
			$reason->ff_id = $ffid; 
			$reason->status = $mode; 
    ...
    public function ViewReImportFF(Request $request){
        ...
        <table class='table table-hover table-bordered'>
			...
			<h5><strong>Reason:</strong> $data->reason</h5>
            </caption>
            <tr>
                <th width='15%'></th>
                <th width='33%'>Generated</th>
	...
	/*start Editing module function*/
    public function editFfData(Request $request){                         
        $admin_id=Auth::guard('admin')->user()->id;
        $record_id=$request['record_id'];
        $ff_name=$request['ff_name'];
        $fh_name=$request['fh_name'];
        $post_office=$request['post_office'];
        $district=$request['district'];
        $ut_name=$request['ut_name']; 
        $vw_name=$request['vw_name'];
        if($ff_name != ''){
			$input_log=array();
			$input_log['ff_name'] = $ff_name;
			$input_log['father_or_husband_name'] = $fh_name;
			$input_log['post_office'] = $post_office;
			$input_log['district'] = $district;
			$input_log['upazila_or_thana'] = $ut_name;
			$input_log['village_or_ward'] = $vw_name;			
			FreedomFighterList::where('id',$record_id)->update($input_log); 
			return response()->json(['success'=>true, 'message'=>'Record has been edited successfully.', 'record_id'=>$record_id]);
        } 
    }

    public function CertificateGeneratePreview(Request $request){
        $admin_info = \Auth::guard('admin')->user()->toArray(); 
        $admin_id=Auth::guard('admin')->user()->id;
        $id=$request['id'];
        $flag=$request['flag'];
        ...
        if(StudentTable::where('serial_no','C_'.$data->ff_id)->where('status',1)->count() == 0){ 
			return response()->json(['success'=>false, 'message'=>'Certificate not yet generated.']); 
			exit();
		}
		$ghostImgArr = array();
        $pdfBig = new \Mpdf\Mpdf(['orientation' => 'P', 'mode' => 'utf-8', 'format' => [296.926, 210.058], 'tempDir'=>storage_path('tempdir')]);
		...
		$fid=$data->id; 
		$ff_id=$data->ff_id;
		$mother_name=$this->IsNullString($data->mother_name);
		...
		$serial_no='C_'.$ff_id;
		if($flag=='Preview'){
			$ff_name=$this->IsNullString($request['ff_name']);
			$fh_name=$this->IsNullString($request['fh_name']);            
			$post_office=$this->IsNullString($request['post_office']);            
			$district=$this->IsNullString($request['district']);
			$ut_name=$this->IsNullString($request['ut_name']);
			$vw_name=$this->IsNullString($request['vw_name']);            
		}else{
			$ff_name=$this->IsNullString($data->ff_name);
			$fh_name=$this->IsNullString($data->father_or_husband_name);            
			$post_office=$this->IsNullString($data->post_office);            
			$district=$this->IsNullString($data->district);
			$ut_name=$this->IsNullString($data->upazila_or_thana);
			$vw_name=$this->IsNullString($data->village_or_ward);				
		}
		...
		*/
		...
		//$dtc = date("_ymdHis");            
		//$str=$serial_no.$dtc;
		$result = StudentTable::where('serial_no',$serial_no)->where('status',1)->first();
		$codeContents =$encryptedString = $result->key;
		...
		if($flag=='Preview'){
			$pdfPath=public_path().'/'.$subdomain[0].'/preview/'.$serial_no.'.pdf';
			$pdfDownloadLink='/'.$subdomain[0].'/preview/'.$serial_no.'.pdf';
			$pdf->output($pdfPath,'F');  //Save individual PDF
		}else{
			$pdfPath=public_path().'/'.$subdomain[0].'/backend/pdf_file/'.$serial_no.'.pdf'; //verification PDF
			$filepath = public_path().'\\'.$subdomain[0].'/molwa_pdfs/certificate/'.$serial_no.'.pdf'; //print PDF
			$pdfDownloadLink='/'.$subdomain[0].'/molwa_pdfs/certificate/'.$serial_no.'.pdf';
			$pdfBig->output($filepath,'F');
			$pdf->output($pdfPath,'F');  //Save individual PDF
			$pdffile=$serial_no.'.pdf';
			$sub_domain=$subdomain[0];
		...
		//$pdf->output($pdfPath,'F');  //Save individual PDF    
        return response()->json(['success'=>true, 'message'=>'<a href="'.$pdfDownloadLink.'" target="_blank">Certificate</a>']);    
    }

    public function IdGeneratePreview(Request $request){
        $admin_info = \Auth::guard('admin')->user()->toArray(); 
        $admin_id=Auth::guard('admin')->user()->id;
        $id=$request['id'];
        $flag=$request['flag'];
        ...
        if($subdomain[0] != "secura"){$subdomain[0]= "secura";}
        $data = FreedomFighterList::where('id',$id)->first();  
        if(StudentTable::where('serial_no','ID_'.$data->ff_id)->where('status',1)->count() == 0){ 
			return response()->json(['success'=>false, 'message'=>'ID not yet generated.']); 
			exit();
		}        
        $pdfBig = new \Mpdf\Mpdf(['orientation' => 'P', 'mode' => 'utf-8', 'format' => [86, 54], 'tempDir'=>storage_path('tempdir')]);
        $pdfBig->SetCreator('seqr'); //PDF_CREATOR
		...
		//$lines = $pdf->MultiCell(45, 0, $ff_name, 0, 'L');
		// restore previous object
		//$pdf=$pdf->rollbackTransaction(true);	
		//$pdf->getNumLines($ff_name, 45);	
		...
		$pdf->SetFont('nikosh', 'B', 9);
		$pdf->SetXY(38.7,20.9);        
		//$pdf->MultiCell(46, 0, $ff_name, 1, 'L');
		$pdf->AutosizeText($ff_name, 46, 'nikosh', 'B', 9 );
		$pdf->SetFont('nikosh', '', 9);
		//Father/Husband Name of FF
		$pdf->SetXY(35.6,24.8);        
		...
		//Upazila/Thana Name
		$pdf->SetXY(39,32.1);
		//$pdf->MultiCell(25, 0, $ut_name, 0, 'L');
		$pdf->AutosizeText($ut_name, 25, 'nikosh', '', 9 );
		//Name of District
		$pdf->SetXY(31,35.7);
		$pdf->MultiCell(55, 0, $district, 0, 'L');         
		...
		if($flag=='Preview'){
			...
			$pdfPath=public_path().'/'.$subdomain[0].'/backend/pdf_file/'.$serial_no.'.pdf'; //verification PDF
			$filepath = public_path().'\\'.$subdomain[0].'/molwa_pdfs/ID/'.$serial_no.'.pdf'; //print PDF
			$pdfDownloadLink='/'.$subdomain[0].'/molwa_pdfs/ID/'.$serial_no.'.pdf';
			$pdfBig->output($filepath,'F');
			$pdf->output($pdfPath,'F');  //Save individual PDF
			$pdffile=$serial_no.'.pdf';
			$sub_domain=$subdomain[0];
        ...
        return response()->json(['success'=>true, 'message'=>'<a href="'.$pdfDownloadLink.'" target="_blank">ID Card</a>']);    
    }	

	public function createghostimg($ff_id, $tmpDir, $name, $ghost_font_size, $subdomain)
	{
		$img_w=0;
		$img_width=992;
		$new_width=10912; //992*11
        $img_height=997;
		$single_char=str_split(trim(strtoupper($ff_id)));
		$length = count($single_char);
		$new_image = imagecreate($new_width, $img_height);
		for ($i = 0; $i < $length; $i++) {
			$char=public_path().'\\'.$subdomain.'\chars\11\\'.$single_char[$i].'.png';
			$img1 = imagecreatefrompng($char);
			imagecopy($new_image, $img1, $img_w, 0, 0, 0, $img_width, $img_height);
			$img_w +=992;
		}		
		imagepng($new_image, $tmpDir."/" . $name."".$ghost_font_size.".png");
	}
	
	public function deleteghostimg($tmpDir)
	{
		$dirPath=$tmpDir;
		if (! is_dir($dirPath)) {
	...
	/*end Editing module function*/

    public function molwaReport(Request $request){
        return view('admin.molwa.report'); 
    }
	public function molwaReportGet(Request $request){
        $admin_id=Auth::guard('admin')->user()->id;        
        $district=$request['district'];
        $upazila=$request['upazila'];
        $is_alive=$request['is_alive']; 
		$report_name=$request['report_name'];
		$pdfData=array('district'=>$district,'upazila'=>$upazila,'is_alive'=>$is_alive,'report_name'=>$report_name);
		$link = $this->dispatch(new PdfGenerateMolwaReport($pdfData));
		return response()->json(['success'=>true, 'message'=>$link]);
    }
