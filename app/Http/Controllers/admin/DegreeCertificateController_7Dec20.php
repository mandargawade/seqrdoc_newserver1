...
class DegreeCertificateController extends Controller
{
    ...
    public function dbUploadfile(){
        ...
        //Test by Bhavin
        $fetch_degree_data_prog_spec = DB::select( DB::raw('SELECT DISTINCT CONCAT_WS(" ",TRIM(Programme_Name_E),TRIM(Specialization_E)) AS prog_spec, COUNT(guid) FROM gu_dc_2 WHERE STATUS="0" GROUP BY prog_spec ORDER BY COUNT(guid) ASC limit 0,10') );
        ...
        {   
            ...
            $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'Degree Certificate',$admin_id);
             ...
             DB::update( DB::raw('UPDATE gu_dc_2 SET status="1" WHERE GUID ="'.$fetch_degree_array[$excel_row][0].'"'));
            }else{

            //valid code
             DB::update( DB::raw('UPDATE gu_dc_2 SET status="2" WHERE GUID ="'.$fetch_degree_array[$excel_row][0].'"'));
