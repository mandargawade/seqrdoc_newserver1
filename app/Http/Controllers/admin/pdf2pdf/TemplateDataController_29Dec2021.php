      ...
      public function index(Request $request)
    {
       ...
       // TemplateMaster::where('create_refresh',1)->update(['create_refresh'=>0]);
        ...
        if($request->ajax()){
            ...
            if($subdomain[0]=="icat"){
                $columns = [DB::raw('@rownum  := @rownum  + 1 AS rownum'),'template_name','pdf_page','id','file_name','publish','template_title', DB::raw('CASE WHEN assigned_labs="0" THEN "" ELSE LENGTH(assigned_labs)-LENGTH(REPLACE(assigned_labs,",",""))+1 END AS LabCount')];
            }else{
                $columns = [DB::raw('@rownum  := @rownum  + 1 AS rownum'),'template_name','pdf_page','id','file_name','publish','template_title'];    
            }
