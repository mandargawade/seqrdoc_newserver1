      ...
      public function index(Request $request)
    {
       ...
       // TemplateMaster::where('create_refresh',1)->update(['create_refresh'=>0]);
        ...
        if($request->ajax()){
            ...
            }elseif($subdomain[0]=="demo"){
                $columns = [DB::raw('@rownum  := @rownum  + 1 AS rownum'),'template_name','pdf_page','id','file_name','publish',DB::raw('IFNULL(`template_title`,"") AS template_title'),'print_bg_file', 'print_bg_status', 'verification_bg_file', 'verification_bg_status', 'is_block_chain'];  
            }
            else{
                ...
                if($pdf_flag==''){
                    if($subdomain[0]=="test"){                      
                       // $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_niit_sikkim.py";
                         $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_test.py";
